...
{ 
	...
	double t=0.0, chi=0.0, zna=0.0, chasc=0.0, chasz=0.0, rs=0.0;
...
{ //расчет лучистого КТП через КП по Росселанду
	int vm=vybmar, vyve=vybves, dlar=0, i=0; 
	int k=0, j=k+1, q=j+1, l=q+1;
	char pbf='A', **mu=napolStrok(vybves, vybmar, pbf);
	char *snm=mu[k], *sfno=mu[j]; if (mu) delete[]mu;
	mu=napNazvFile(vybves, vybmar, snm, pbf);
	char *szfov=mu[k], *sppov=mu[j], *skpov=mu[q], *sdvov=mu[l]; if (mu) delete[]mu;
	//-----
	...
	double dko2=dkosups, dko4=dkokpiko, dko=dko2*dko4; //учет пористой структуры и соотношения КП/КО //cout << "szfov = " << szfov << endl;
	double **unau=dliny_voln(snm, sdvov, sfno); //cout << "sfno = " << sfno << endl;
	double *dl=unau[k], *ne=unau[j], e=1e-9, d=e, t=ne[k]; 
	while (d<t) { d=d+hf; i++; } dlar=i; if (ne) delete[]ne; if (unau) delete[]unau;
	if (dko<0.0) dko=0.0; if (dko>hf) dko=hf; //cout << "dlar = " << dlar << endl;
	//-----
	...
	npp=Kramers_Kronig(dl, dlar, npp, sppov, sfno, snm, vybves, vybmar, dlar, skpov, dko); //for (k=0; k<(dm/50); k++) cout << alfs[k] << "\t"; for (k=0; k<(dm/50); k++) cout << npp[k] << "\t"; 
	...
	npT[k]=usredVelichPlank(dl, npp, npp, temk, dlar, hf, snm); //cout << "al_sred = " << usredVelichPlank(dl, alfs, npp, temk, dlar, hf, snm) << "\t"; //dkusct=opredKTPTKToch(dkusctm, tdkusctm, temk, dkusctl); dkosce=opredKTPTKToch(dkoscem, dkoscet, temk, dkoscel);
	krpk=KoefPoglRossel(temk, dl, alfs, npp, dlar, snm); kpr[k]=krpk; //cout << "\ttemp = " << temk << "\tKoef_pogl_Ross = " << krpk << endl; 
	} 
	...
	ktr[k]=krpk; //cout << "\ttemp = " << temk << "\tn_sr = " << npT[k] << "\tKoef_Pogl_Ross = " << ktr[k] << "\t"; 
	} cout << endl;
	if (dl) delete []dl; if (npp) delete []npp; if (alfs) delete []alfs; 
	if (kpr) delete []kpr; if (GnpT) delete []GnpT; if (npT) delete []npT; 
	if (snm) delete[]snm; if (sfno) delete[]sfno; if (szfov) delete[]szfov; 
