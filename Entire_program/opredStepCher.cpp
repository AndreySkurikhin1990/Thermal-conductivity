...
//-------------------
char **napolStrok(int vyve, int vm, char pbf)
{	
	int k=0, dsov=60, n=2; 
...
char **napNazvFile(int vyve, int vm, char *snm, char pbf)
{
	...
	char *skpovk=new char[dsov], *sdvovk=new char[dsov], *sppovk=new char[dsov];
	char *skpov=new char[2*dsov], *sdvov=new char[2*dsov], *sppov=new char[2*dsov];
...
for (k=0; k<dsov; k++) { skpovk[k]='\0'; sdvovk[k]='\0'; sppovk[k]='\0'; sndov[k]='\0'; ssdov[k]='\0'; szfovk[k]='\0'; }
k=0; 
sndov[k]='D'; k++; sndov[k]=':'; k++; sndov[k]='\\';k++; sndov[k]='\\'; k++; sndov[k]='_';  k++; sndov[k]='ј';  k++;
...
ssdov[k]='e';  k++; ssdov[k]='r'; k++; ssdov[k]='s';  k++; ssdov[k]='\\'; k++; ssdov[k]='\\'; k++; ssdov[k]='ј';  k++;
...
ssdov[k]='_';  k++; ssdov[k]='ј'; k++; ssdov[k]='с';  k++; ssdov[k]='п';  k++; ssdov[k]='и';  k++; ssdov[k]='р';  k++;
...
sppovk[k]='.'; k++; sppovk[k]='t'; k++; sppovk[k]='x'; k++; sppovk[k]='t'; k++; sppovk[k]='\0'; //0 - шамот, 1 - вермикулит, 2 - »“ќћ, 3 -  ¬»
...
double epsisred(double T, double *tdkusctm, double *dkusctm, int dkusctl, double *dkoscet, double *dkoscem, int dkoscel, int vybves, int vybmar, double dko)
{ 
...
for (k=0; k<dlar; k++) npp[k]=0.0; //cout << "szfov = " << szfov << endl; 
...
double epssr=usredVelichPlank(dv, epsil, npp, T, dlar, ume, snm); //cout << "T = " << T << "\teps_s = " << epssr << "\tume = " << ume << "\n";
for (k=0; k<=j; k++) { snm=mauk[k]; if (snm) delete[]snm; } if (mauk) delete[]mauk; 
for (k=0; k<=l; k++) { snm=mu[k]; if (snm) delete[]snm; } if (mu) delete[]mu; //if (snm) delete[]snm; if (sfno) delete[]sfno; if (szfov) delete[]szfov; if (szfov) delete[]szfov; cout << "k = " << k << endl; if (sppov) delete[]sppov; if (skpov) delete[]skpov; if (sdvov) delete[]sdvov; cout << "k = " << k << endl; 
...
if (!fo.is_open()) { cout << sfno << endl; k=getchar(); exit(1); }
	...
	double *alsr=new double[lear], e=1e-15, ht=1e0, ko=1e-2, enu=1e-3; //for (k=0; k<lear; k++) alsr[k]=0.0;
...
nnus=PokazPrelomAl(nu, nus, alsr, c0, lear, nnus, snm); //for (k=0; k<lear; k++) cout << "k = " << k << "\tnu = " << nnus[k] << endl; cout << "lear = " << lear << endl; k=getchar();
...
for (k=0; k<dlar; k++) alpha[k]=0.0; //cout << "ce = " << dlar << endl;
