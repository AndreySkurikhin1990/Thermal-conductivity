...
void PrintMatr(double **mas, int m) { // Ôóíêöèÿ âûâîäà ìàòðèöû
...
double vychopred(double  **mas, int m) {
  ...
  return d; }
void vyvodmatr(double **a, double *b, int n) // Âûâîä ñèñòåìû óðàâíåíèé
...
k=0; while (k<n) { // Ïîèñê ñòðîêè ñ ìàêñèìàëüíûì a[i][k]
	...
	} //íåò íåíóëåâûõ äèàãîíàëüíûõ ýëåìåíòîâ
    for (j=0; j<n; j++) { temp=a[k][j]; a[k][j]=a[index][j]; a[index][j]=temp; } temp=y[k]; y[k]=y[index]; y[index]=temp; //Ïåðåñòàíîâêà ñòðîê
    for (i=k; i<n; i++) {
      temp=a[i][k]; if (!(fabs(temp))) continue; // äëÿ íóëåâîãî êîýôôèöèåíòà ïðîïóñòèòü
...
for (k=n-1; k>=0; k--) { // îáðàòíàÿ ïîäñòàíîâêà
