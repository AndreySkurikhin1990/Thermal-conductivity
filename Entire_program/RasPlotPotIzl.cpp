...
{ cout << snm << endl; k=getchar(); exit(1); }
    ...
    i=0; j=4; k=1; u=2; w=3; Fs[i][j]=0.5*(hf-(hf-RHOs[k])*Fs[i][k]-Fs[i][u]-Fs[i][w]); //смещены два прямоугольника на ww по оси X
	...
	u=2; w=3; Fs[i][j]=0.5*(hf-(hf-RHOs[k])*r-Fs[i][u]-Fs[i][w]); //Fs25
	...
	u=1; w=3; Fs[i][j]=0.5*(hf-(hf-RHOs[k])*r-(hf-RHOs[u])*t-Fs[i][i]-Fs[i][w]);
	...
	u=1; Fs[i][j]=0.5*(hf-((hf-RHOs[k])*r+(hf-RHOs[u])*t+b)/A[i]-Fs[i][i]);
	...
	sumq=0.0; //проверка полной ППИ
		...
		} r=0.0; z=hf; j=0; for (i=0; i<N; i++) if (ns[j]==i) { t=q[i]; vm[j]=t; j++; r=r+z*t; z=-z; } //cout << "Summa vsekh teplovykh potokov = " << sumq << endl;
		for (i=0; i<N; i++) { p=Fs[i]; if (p) delete[]p; } if (Fs) delete[]Fs; 
		if (PIN) delete []PIN; if (POUT) delete[]POUT; if (arg) delete[]arg; 
		if (QA) delete[]QA; if (id) delete[]id; 
		if (!v) { if (q) delete[]q; return vm; }
		else { if (vm) delete[]vm; return q; } }
...
for (i=1; i<N; i++)
	...
	if (iclsd==1) { for (i=0; i<N; i++) {
	 ...
	 for (i=0; i<N; i++) { // Заполнение матриц коэффициентов q и e
		for (j=0; j<N; j++) {
			if (i==j) ikr=hf; else ikr=0.0; // Символ Кронекера delta_ij //cout << "i = " << i << "\tj = " << j << "\tk = " << ikr << endl;
			if (fabs(EPS[j])>epsi) 
	 ...
	 for (i=0; i<N; i++) { sumq=0.0; for (j=0; j<N; j++) sumq=sumq+Fs[i][j]; /*cout << "Proverka = " << sumq << "\t";*/ }
	 POUT=MetodGaussa(pm, B, N, snm); //for (i=0; i<N; i++) cout << "pout = " << POUT[i] << endl; cout << endl; POUT=GAUSS(pm, B, N, POUT); //for (i=0; i<N; i++) cout << "pout = " << POUT[i] << "\t"; cout << endl;
	 ...
	 if (T) delete []T; if (q) delete []q; if (QA) delete []QA; //for (i=0; i<N; i++) { for (j=0; j<N; j++) printf("%0.6lf\t", Fs[i][j]); cout << endl; }
	 for (i=0; i<N; i++) { p=qm[i]; if (p) delete []p; p=em[i]; if (p) delete []p; p=pm[i]; if (p) delete []p; } 
...
for (i=1; i<N; i++)
	...
	if (iclsd==1) { for (i=0; i<N; i++) {
	 ...
	 for (i=0; i<N; i++) { // Заполнение матрицы выходных коэффициентов POUT и RHS
			...
			for (j=0; j<N; j++) {
					pm[i][j]=qm[i][j]*idf[j]-em[i][j]*(hf-idf[j]);
					B[i]=B[i]+(em[i][j]*idf[j]-qm[i][j]*(hf-idf[j]))*PIN[j]; } } //cout << "Fs" << endl; for (i=0; i<N; i++) { for (j=0; j<N; j++) printf("%0.4lf\t",Fs[i][j]); cout << endl; } //for (i=0; i<N; i++) { y=0.0; for (j=0; j<N; j++) y=y+(1e0-RHOs[j])*Fs[i][j]; cout << "stroka ( " << i << " ) = " << y << endl; } //j=getchar();
	 POUT=MetodGaussa(pm, B, N, snm); //for (i=0; i<N; i++) cout << "pout = " << POUT[i] << endl; cout << endl; POUT=GAUSS(pm, B, N, POUT); for (i=0; i<N; i++) cout << "pout = " << POUT[i] << endl; cout << endl; POUT=reshMetObrMatr(pm, B, N, POUT); for (i=0; i<N; i++) cout << "pout = " << POUT[i] << endl; cout << endl; //for (i=0; i<N; i++) { for (j=0; j<N; j++) printf("%0.10lf\t", Fs[i][j]); cout << endl; }
	 double sumq=0.0, *T=new double[N], sigma=5.67e-8, *q=new double[N], *QA=new double[N];
	for (i=0; i<N; i++) { if (!(ID[i])) T[i]=pow((POUT[i]/sigma),1e0/4e0); else q[i]=POUT[i];
...
double *SEMIGRAY(int iclsd, double *A, double **EPS, double **RHOs, double **HOs, double ***Fs, int *ID, double *q, double *T, double *PIN, 
	double *POUT, int chst, int chel, char *snm)
{ int N=chst, i=0, j=0, k=0, *id1=new int[N];
double ikr=0.0, sigma=5.670E-8, *epsl=new double[N], *rhosl=new double[N], hf=1e0;
double **FSl=new double*[N], *q1=new double[N], *HOs2=new double[N], *p=NULL; // Расчет плотности теплового потока  для внешнего излучения (для кажого диапазона)
for (j=0; j<N; j++) { p=new double[N]; FSl[j]=p; }
	...
	GRAYDIFFSPEC(iclsd, A, epsl, rhosl, HOs[k], FSl, id1, PIN, q1, N, snm); //Вычисление неизвестных плотностей потоков тепловой энергии и температур при q2=q-q1
    k=1; for (i=0; i<N; i++) {
        ...
        if (iclsd==1) {
            ...
            for (j=0; j<N; j++) 
				if (j!=i)
                    FSl[i][i]=FSl[i][i]-(hf-rhosl[j])*FSl[i][j]; }
        HOs2[i]=-q1[i]/EPS[k][i];
        for (j=0; j<N; j++) HOs2[i]=HOs2[i]+((hf-rhosl[j])/EPS[2][j]-hf)*FSl[i][j]*q1[j];
        if (!(ID[i])) PIN[i]=q[i];
		else PIN[i]=sigma*pow(T[i],4e0); }
    GRAYDIFFSPEC(iclsd, A, epsl, rhosl, HOs2, FSl, ID, PIN, POUT, N, snm);
    for (i=0; i<N; i++) {
        if (!(ID[i])) T[i]=pow((POUT[i]/sigma), hf/4e0);      
        else q[i]=POUT[i]; } 
	if (id1) delete[]id1; if (epsl) delete[]epsl; if (rhosl) delete[]rhosl; 
	if (q1) delete[]q1; if (HOs2) delete[]HOs2;
	for (j=0; j<N; j++) { p=FSl[j]; if (p) delete[]p; } delete []FSl;
	return POUT;
}
...
if (NO==38) { 
	...
	if (NARG!=3) { cout << "Wrong number of input parameters (NARG =" << NARG << ") for NO = " << NO << endl; k=getchar(); exit(1); }
		...
		B=pow(X*X-4.0*B, ov);
        VIEWF=ov*(X-B); }
else if (NO==41) { //внутренняя поверхность на основание
...
double *SeryeStenkiRasIzlDifPov(double ww, double hh, double ll, double *T, double *EPS, double *HOs, double *A, int *ns, int chst, 
	char *snm) //N серых диффузных поверхностей
{ 
	int N=chst, fl=0; double b=0.0, t=0.0, epsi=1e-20; //t=T[ns[0]]; b=T[ns[1]]; if (fabs(t-b)<epsi) return 0.0;
double **Fs=new double*[N], z=0.0, r=0.0, y=0.0, hf=1e0; int i=2, j=0, f=3, k=0, d=0; 
double *PIN=new double[N], *POUT=new double[N], *arg=new double[f], *p=NULL, *vm=new double[i];
for (i=0; i<N; i++) { p=new double[N]; if (!p) { cout << snm << endl; k=getchar(); exit(1); } 
for (j=0; j<N; j++) p[j]=0.0; Fs[i]=p; }
	double sumq=0.0, sigma=5.67e-8, *q=new double[N], *QA=new double[N];
	...
	{ cout << snm << endl; k=getchar(); exit(1); }
    	for (i=0; i<N; i++) if (!(id[i])) PIN[i]=q[i]; else PIN[i]=sigma*pow(T[i], 4e0); //Заполнение массива PIN ПТП q и температурой T - перевод температур в ППИ, Fs(1,2)=F1-2, 
	iclsd=1; //угловые коэффициенты; конфигурация замкнута (iclsd=1), диагональные элементы не нужны for (j=0; j<N; j++) cout << "EPS ( " << j << " ) = " << EPS[j] << endl;
	k=0; arg[k]=fabs(hh); k++; arg[k]=fabs(ll); k++; arg[k]=fabs(ww); //h,l,w
	...
	k=0; arg[k]=fabs(ww); k++; arg[k]=fabs(hh); k++; arg[k]=fabs(ll);
    k=38; i=4; j=5; Fs[i][j]=VIEW(k, f, arg, fl); //F56 //PrintMatr(Fs, N); for (j=0; j<N; j++) { r=0.0; for (k=0; k<N; k++) r=r+EPS[k]*Fs[j][k]; cout << "s = " << r << "\t"; } cout << endl; for (j=0; j<N; j++) { r=0.0; for (k=0; k<N; k++) r=r+Fs[j][k]; cout << "s = " << r << "\t"; } k=getchar();
	POUT=GRAYDIFF(iclsd, A, EPS, HOs, Fs, id, PIN, POUT, N, snm);
	sumq=0.0; //проверка полной ППИ
    	for (i=0; i<N; i++) //Вывод - перевод в температуры
		{ if (!(id[i])) T[i]=pow((POUT[i]/sigma), 1e0/4e0); else q[i]=POUT[i];
        	QA[i]=q[i]*A[i]; sumq=sumq+QA[i]; //cout << "Surface " << i << "\tT [K] = " << T[i] << "\tq [W/m2] = " << q[i] << "\tQA [W] = " << QA[i]/ll << endl;
		} r=0.0; z=hf; j=0; for (i=0; i<N; i++) if (ns[i]==i) { t=q[i]; vm[j]=t; j++; r=r+z*t; z=-z; } 
		for (i=0; i<N; i++) { p=Fs[i]; if (p) delete[]p; } if (Fs) delete[]Fs; if (PIN) delete[]PIN; 
		if (POUT) delete[]POUT; if (arg) delete[]arg; if (q) delete[]q; if (QA) delete[]QA; if (id) delete[]id; //cout << "Summa vsekh teplovykh potokov = " << sumq << endl;
		return vm; }
double F0_lamT(double lT0, char *snm)
...
char pbf='\0', **mu=napolStrok(vybves, vybmar, pbf);
char *snm=mu[k], *sfno=mu[j], *szfov=mu[q]; 
char **mauk=napNazvFile(vybves, vybmar, snm, pbf);
char *ps=mauk[k], *sppov=mauk[j], *skpov=mauk[q], *sdvov=mauk[l]; 
	double **unau=dliny_voln(snm, sdvov, sfno);
	double *dv=unau[k], *ne=unau[j], e=1e-9, d=e, t=ne[k]; 
...
if (npp) delete []npp; if (dv) delete []dv; //if (snm) delete[]snm; if (sfno) delete[]sfno; if (szfov) delete[]szfov;  if (ps) delete[]ps; if (sppov) delete[]sppov; if (skpov) delete[]skpov; if (sdvov) delete[]sdvov;
for (k=0; k<l; k++) { snm=mu[k]; if (snm) delete[]snm; } if (mu) delete[]mu;
for (k=0; k<=l; k++) { snm=mauk[k]; if (snm) delete[]snm; } if (mauk) delete[]mauk; 
return t; }
...
for (j=0; j<N; j++) p[j]=0.0; Fs[i]=p; }
	...
	k=43; i=0; j=1; Fs[i][j]=VIEW(k, f, arg, fl); //F12
    k=40; i=0; j=2; Fs[i][j]=VIEW(k, f, arg, fl); //F13
    k=42; i=1; j=i; Fs[i][j]=VIEW(k, f, arg, fl); //Fs22
	k=41; i=1; j=2; Fs[i][j]=VIEW(k, f, arg, fl); //Fs23
	POUT=GRAYDIFF(iclsd, A, EPS, HOs, Fs, id, PIN, POUT, N, snm);
	sumq=0.0; //проверка полной ППИ
    	for (i=0; i<N; i++) //Вывод - перевод в температуры
		{ if (!(id[i])) T[i]=pow((POUT[i]/sigma), 1e0/4e0); else q[i]=POUT[i];
        	QA[i]=q[i]*A[i]; sumq=sumq+QA[i]; //cout << "Surface " << i << "\tT [K] = " << T[i] << "\tq [W/m2] = " << q[i] << "\tQA [W] = " << QA[i]/ll << endl; 
		} g=0.0; z=hf; j=0; for (i=0; i<N; i++) if (ns[j]==i) { t=q[i]; vm[j]=t; j++; g=g+z*t; z=-z; } //cout << "Summa vsekh teplovykh potokov = " << sumq << endl;
		for (i=0; i<N; i++) { p=Fs[i]; if (p) delete[]p; } if (Fs) delete[]Fs; 
		if (PIN) delete []PIN; if (POUT) delete[]POUT; if (arg) delete[]arg; 
		if (QA) delete[]QA; if (id) delete[]id; 
		if (!v) { if (q) delete[]q; return vm; }
