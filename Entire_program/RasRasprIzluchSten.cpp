...
{ //расчет коэффициентов излучения, дошедшего от j до k-ой стенки
	...
	for (k=0; k<f; k++) prs[k]=0.0;
		...
		for (k=0; k<m; k++) vm[k]=preobMas(prs, m, kost, k); 
		//-----
		if (pr) delete[]pr; if (arg) delete[]arg; if (prs) delete[]prs; 
...
{
	int k = 3, j=0, q = 2; 
	double *khrl=new double[kost], *khlr=new double[kost], *ri=new double[q], **mu=new double*[k], e=1e-12;
	double *hrl1 = new double[kost], *hlr1 = new double[kost], hf=1e0;
	double **mu0=NULL, **mu1=NULL, **mu2=NULL, **mu3=NULL, ta=0.0, tb=0.0;
	...
	for (j = 0; j < kost; j++) { //узнаем, какая доля внешнего излучения дошла до j-ой стенки - учитываем вклад от всех стенок
		tlr = 0.0; trl = 0.0; //внешнее излучение падает с обеих сторон
		for (k = 0; k<kost; k++) {
			w0=mu0[j][k]; 
			w1=mu1[j][k]; 
			w2=mu2[j][k]; 
			...
			ta=Ref[k] * w2;
			tb=Tra[k] * w3;
			tt = ta + tb; //доля энергии, отраженная от k-ой стенки, и прошедшая через нее, пришла справа к j-ой стенке
			if (fabs(wk)>e) {
				ta=tt*khlr[k] * w0 / wk; //доля энергии, отраженная от k-ой стенки, и прошедшая через нее, пришла слева к j-ой стенке
				tb=tt*khlr[k] * w1 / wk;
			} 
			else { ta=0.0; tb=0.0; }
			tlr = tlr + ta;
			trl = trl + tb; //для учета при подсчете лучистой энергии, распространяющейся справа налево
	} 
		tlr=tlr*hlr0;
		hlr1[j]=hlr1[j]+tlr;
		trl=trl*hlr0;
		hrl1[j]=hrl1[j]+trl; } 
	for (j=kost-1; j>=0; j--) { //считаем, что стенки, до которых дошло излучение, как бы "сами" излучают, из-за того, что на них попало внешнее излучение
		for (k = (kost - 1); k >= 0; k--) {
			w0=mu0[j][k]; 
			w1=mu1[j][k]; 
			w2=mu2[j][k]; 
			...
			ta=Ref[k] * w3;
			tb=Tra[k] * w2;
			tt = ta + tb; //деление на два потока
			if (fabs(wk) > e) {
				ta=tt*khrl[k]*w0/wk; //для учета лучистой энергии, распространяющейся слева направо, падает слева от j-ой стенки
				tb=tt*khrl[k]*w1/wk;
			} 
			else { ta=0.0; tb=0.0; } 
			tlr=tlr+ta; 
			trl=trl+tb; //то, что попадает справа налево на стенку, справа от j-ой стенки
		} 
		tlr=tlr*hrl0;
		hlr1[j]=hlr1[j]+tlr; //узнаем величину в абсолютных единицах этой лучистой энергии - слева от пластинки, падает слева направо
		trl=trl*hrl0;
		hrl1[j]=hrl1[j]+trl; //справа от пластинки, падает справа налево
	} 
	k=kost-1; slr = slr + Tra[k] * hlr1[k] + hrl0*Ref[k]; k=0; ri[k] = slr; //то, что вышло из последней стенки вправо
	k=0; srl = srl + Tra[k] * hrl1[k] + Ref[k] * hlr0; k=1; ri[k] = srl; //то, что вышло из первой стенки влево
...
double *RasIzlSerStenNacSok(double *Refl, double *Tran, double *Abso, double ***mpov, double ***mpos, double temc, double hlr0, double hrl0, 
	double poristo, double dpctp, double htch, double hvoz, int kost, double *ete, double *kttkve, int cemve, double *ktpvove, double *tevove, 
	char *snm, double *stchsrve, int dmkvove, double qobvee, double hko, double p1, double p2, double tx, double dkosups, double dkokpiko, 
	int vybves, int vybmar, int vp,	double *ktr, int dmktr, double epstem, int nomst, int indic, int vfqi, int kolPok, int vogf)
{
	int j=2, k=0, f=6, *nost=new int[j], ki=0, m=3, u=1, x=4, y=5, v=0, q=0, n=2*kost, kl=0, kp=2;
	double *qi=NULL, hf=1e0, d=0.0, w2=d, w3=d, w0=d, w1=d, wo=d, rc=p1*hvoz/2e0;
	...
	if ((!tvs) || (!epst) || (!hvi) || (!Rs) || (!Ap) || (!qil) || (!qir) || (!Tpctve)) 
	{ cout << snm << endl; k = getchar(); exit(1); }
	d=0.0; for (j=0; j<kost; j++) d=d+hf;
	if (!vogf) f=6; else if (vogf==1) f=3;
	k=0; nost[k]=kl; k++; nost[k]=kp;
	...
	dt=opredTempStenFragm(tx, n, ktpvove, tevove, ete, kttkve, cemve, dmkvove, htch, hvoz, qobvee, mo); 
	j=0; hlr0=ktr[j]*dt/t; } 
	r=(p1+p2)*hvoz/2e0; //поперечный размер поры
	s=r*dpctp/(hf-2e0*dpctp); //поперечный размер перемычки
	s=r/(r+2e0*s); s=pow(s, 2e0); s=hf-s; //доля поперечной площади, через которую идет чистая теплопроводность
	Tpctve=opredTempTvKarFragm(kost, ete, kttkve, cemve, htch, hvoz, s*qobvee, temc, mo, snm, tx, Tpctve); //for (k=0; k<kost; k++) cout << "t_v_s ( " << k << " ) = " << Tpctve[k] << "\t"; cout << endl;
	s=hf-s; //доля поперечной площади многслойной стенки
	k=0; Ts=opredTempStenFragmMasTem(n, ktpvove, tevove, ete, kttkve, cemve, dmkvove, htch, hvoz, s*qobvee, temc, mo, snm, kost, k); 
	k++; Tss=opredTempStenFragmMasTem(n, ktpvove, tevove, ete, kttkve, cemve, dmkvove, htch, hvoz, s*qobvee, temc, mo, snm, kost, k); //for (k=0; k<kost; k++) cout << "t_l_s ( " << k << " ) = " << Ts[k] << "\tt_p_s = " << Tss[k] << "\t"; cout << endl;
	if ((epstem>e) && (indic>=0)) 
	{ if (!(indic)) Ts[nomst]=Ts[nomst]+epstem; 
	else Tss[nomst]=Tss[nomst]+epstem; } //добавка к аргументу для поиска производной
	...
	k=0; sirv=reiz[k]; k++; silv=reiz[k]; if (reiz) delete[]reiz; //for (k=0; k<kost; k++) cout << "silr ( " << k << " ) = " << sir[k] << "\tsirl = " << sil[k] << "\t"; cout << endl;
	k=0; Ap[k]=w*l; //k=0
	k++; Ap[k]=h*l; //k=1
	k++; ki=0; Ap[k]=Ap[ki]; //k=2
	k++; ki++; Ap[k]=Ap[ki]; //k=3
	k++; ki=k; Ap[k]=w*h; //x=4
	k++; Ap[k]=Ap[ki]; //y=5 //k=4*kost*(kost-1); ki=4*kost*kost; for (j=k; j<ki; j++) { if (!(j%4)) cout << endl; cout << "w ( " << j << " ) = " << mpo[j] << "\t"; } //for (k=0; k<2; k++) cout << "ns ( " << k << " ) = " << nost[k] << "\t"; cout << endl;
	for (j=0; j<(kost-1); j++) {
		for (k=0; k<f; k++) tvs[k]=Tpctve[j]; 
		k=kl; 
		tvs[k]=Tss[j]; 
		k=kp; 
		tvs[k]=Ts[j+1]; //0 - более высокая температура у стенки, 2 - менее высокая
		for (k=0; k<f; k++) { 
			t=opredKTPTKToch(stchsrve, ete, tvs[k], cemve); 
			if (t<0.0) t=0.0; if (t>hf) t=hf; 
			epst[k]=t; //cout << "eps ( " << k << " ) = " << epst[k] << "\t";
			Rs[k]=hf-t;
			hvi[k]=0.0; } 
		k=kl; 
		hvi[k]=hrl1[j]+sil[j]; 
		epst[k]=Abso[j]; Rs[k]=Refl[j]; //t=Abso[j]; r=Refl[j]; s=t/(t+r); epst[k]=s; Rs[k]=hf-s;
		k=kp; hvi[k]=hlr1[j+1]+sir[j+1]; 
		epst[k]=Abso[j+1]; Rs[k]=Refl[j+1]; //t=Abso[j+1]; r=Refl[j+1]; s=t/(t+r); epst[k]=s; Rs[k]=hf-s;
	v=0; 
	if (!vogf) { 
		...
		else if (vfqi==1) qi=SeryeStenkiRasIzlDifPov(w, h, l, tvs, epst, hvi, Ap, nost, f, snm, v); } //N серых диффузных поверхностей
	else if (vogf==1) qi=SeryeStenkiRasIzlCyl(rc, hvoz, tvs, epst, hvi, Rs, Ap, nost, f, snm, v);
	...
	k=kost-1; qir[k]=-(slr+sirv); //приходит справа (налево) на последнюю стенку //for (k=0; k<kost; k++) cout << "ql ( " << k << " ) = " << qil[k] << "\tqr = " << qir[k] << "\t"; cout << endl; //for (k=0; k<f; k++) cout << "Ap ( " << k << " ) = " << Ap[k] << "\tRs = " << Rs[k] << "\teps = " << epst[k] << "\thv = " << hvi[k] << endl; //k=getchar();
	if (tvs) delete[]tvs; if (epst) delete[]epst; if (hvi) delete[]hvi; 
...
double **RaschSobLuchPlotTepPot(int kost, double *Ts, double *Tss, double *Tra, double *Ref, double *Ab, char *snm, double ***prot)
{
	...
	for (j=0; j<kost; j++) { sislr[j]=0.0; sisrl[j]=0.0; }
	for (j=0; j<kost; j++) { //узнаем, какая энергия собственного излучения дошла до j-ой стенки
		...
		for (k=0; k<kost; k++) {
			...
			tt=ta+tb;
			if (fabs(wk)>e) {
				ta=sig*Ab[k]*w0*tt/wk; // то, что попадает слева направо
				tb=sig*Ab[k]*w1*tt/wk; } 
			else { ta=0.0; tb=0.0; }
			tlr=tlr+ta; 
			trl=trl+tb; // то, что попадает справа налево
		}
		sislr[j]=sislr[j]+tlr; //собственное излучение стенок слева направо и справа налево
		sisrl[j]=sisrl[j]+trl;
	}
	k=kost-1; 
	slr=slr+Tra[k]*sislr[k]; 
	slr=slr+Ab[k]*sig*pow(Tss[k], 4e0); //из последней стенки
	k=0; 
	srl=srl+Tra[k]*sisrl[k]; 
	srl=srl+Ab[k]*sig*pow(Ts[k], 4e0); //из первой стенки //стенок вне многослойной стенки нет, излучение ниоткуда не падает, есть только внешнее излучение //cout << "slr = " << slr << "\tsrl = " << srl << "\tra = " << ra << "\t"; //излучение за крайней правой стенкой и за крайней левой
