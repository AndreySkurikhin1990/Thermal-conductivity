...
double *poisMasKoefItom(int no, double *kti, int n, char *snmi)
{
	...
	if (!no) {
		...
		ktpit[w] = 12.0*1e-2; ktpit[v] = 139.0*1e-3; //из Диссертации
		//ktpit[w] = 18.0*kk; ktpit[v] = 19.0*kk; //Данные 2017 года
		...
		ktpit[w] = 18.3*kk; ktpit[v] = 19.4*kk; //из Диссертации
		...
		ktpit[w] = 23.0*kk; ktpit[v] = 25.0*kk; //из Диссертации
