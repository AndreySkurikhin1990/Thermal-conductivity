...
double *opredKoefOtrEdin(int vybves, int vybmar, char *snm, double dko, double *tem, int dmko, int vyb)
{
	...
	for (k=0; k<dmko; k++) ko[k]=ReflSred(tem[k], vybves, vybmar, dko, vyb);
	return ko;
}
...
for (k=0; k<=j; k++) { snm=mauk[k]; if (snm) delete[]snm; } if (mauk) delete[]mauk; //cout << "R_sr = " << t << "\t"; 
for (k=0; k<=l; k++) { snm=mu[k]; if (snm) delete[]snm; } if (mu) delete[]mu; //cout << "T = " << T << "\t"; 
if (dv) delete []dv; if (npp) delete []npp; if (ronu) delete []ronu; //cout << "da = " << dlar << "\t"; //if (snm) delete[]snm; if (sfno) delete[]sfno; if (szfov) delete[]szfov; if (ps) delete[]ps; if (sppov) delete[]sppov; if (skpov) delete[]skpov; if (sdvov) delete[]sdvov;
return t; }
