...
//----------------//Çàäàíèå ðàñïðåäåëåíèÿ ïîð ïî ðàçìåðàì - ÈÒÎÌ
...
{ int n=dmsrpi, k=0, f=cvym, j=0, qg=0; //÷èñëî âûõîäíûõ ìàññèâîâ
...
if (legr00[p]<=ht) m=raspr0[p]*(ht-legr00[p])/(prgr00[p]-legr00[p]); //ðàçìåð ïîð äî 1 ìêì
...
fl=1; t=0.0; for (j=0; j<qg; j++) { for (u=0; u<n; u++) if (fabs(legr00[u]-t)>e) { fl=-1; break; } if (fl<0) break; t=t+ht; } //fl - ìåæäó öåëûìè - ôàêò ñóùåñòâîâàíèÿ
...
{ 
int k=0;
if (!vvi) r=rasPorpoRazmitom440(r, n); //ÈÒÎÌ-440
else if (vvi==1) r=rasPorpoRazmitom620(r, n); //ÈÒÎÌ-620
else if (vvi==2) r=rasPorpoRazmitom860(r, n); //ÈÒÎÌ-860
else if (vvi==3) r=rasPorpoRazmitom1000(r, n); //ÈÒÎÌ-1000
return r;
}
...
for (k=0; k<n; k++) srra[k]=(legr[k]+prgr[k])/2e0; /*ñðåäíèé ðàçìåð êàæäîãî èç äèàïàçîíîâ*/
k=0; mu[k]=legr; k++; mu[k]=prgr; k++; mu[k]=raspr; k++; mu[k]=srra; k++; mu[k]=rpr;
return mu; }
...
for (k=0; k<n; k++) prgr[k]=prgr[k]*ko; /*â ìåòðàõ*/ 
return prgr; }
...
for (k=0; k<n; k++) srra[k]=(legr[k]+prgr[k])/2.0; /*ñðåäíèé ðàçìåð êàæäîãî èç äèàïàçîíîâ*/ 
k=0; mu[k]=legr; k++; mu[k]=prgr; k++; mu[k]=raspr; k++; mu[k]=srra; k++; mu[k]=rpr;
return mu; }
...
{ int k=0;
if ((!vysove) || (vysove==2)) { 
if (!vfv) raspr=rasPorpoRazmVerI207(raspr, dmsrpv); //äëÿ ôðàêöèè 2-0,7 ìì, èñõîäíûé
else if (vfv==1) { if (!vpkf) raspr=rasPorpoRazmVerIs84(raspr, dmsrpv); //äëÿ ôðàêöèè 8-4 ìì, èñõîäíûé
else if (vpkf==1) raspr=rasPorpoRazmVerIn84(raspr, dmsrpv); } //äëÿ ôðàêöèè 8-4 ìì, èñõîäíûé, äðóãàÿ ïîðèñòîñòü
else if (vfv==2) raspr=rasPorpoRazmVerI16035(raspr, dmsrpv); } //äëÿ ôðàêöèè 1,6-0,35 ìì, èñõîäíûé
if (vysove==1) {
if ((vfv==2) || (!vfv)) raspr=rasPorpoRazmVerO16035(raspr, dmsrpv); //äëÿ ôðàêöèè 1,6-0,35 ìì è 2-0,7 ìì, ïîñëå îáæèãà
else if (vfv==1) raspr=rasPorpoRazmVerO84(raspr, dmsrpv); //äëÿ ôðàêöèè 8-4 ìì, ïîñëå îáæèãà
else { cout << "Net takoy fraktsii!"; k=getchar(); exit(1); } } //òîëüêî äëÿ ôðàêöèè 8-4 ìì
return raspr; }
...
return mu; } //ñðåäíèé ðàçìåð êàæäîãî èç äèàïàçîíîâ
...
double **rasPorpoRazVer(double poris, int vfv, int vyvyzn, int vysove, int isrp, int vpkf) //0 - ñòàðûå, 1 - íîâûå çíà÷åíèÿ
...
if (isrp==1) { //ðàñ÷åò õàðàêòåðèñòèê ïîðèñòîé ñòðóêòóðû ïðè óñëîâèè øàðîîáðàçíûõ ïîð
...
s=0.0; for (k=0; k<n; k++) if (prgr00[k]<(ht+e)) { p=k; s=s+raspr0[k]; } //ðàçìåð ïîð äî 1 ìêì
double *ras01=new double[qg], *prgrm=new double[qg], *legrm=new double[qg];
...
{ 
int k=0, f=cvym; //÷èñëî âûõîäíûõ ìàññèâîâ
double **mu=new double*[f]; if (!mu) { cout << "No memory!" << endl; k=getchar(); exit(1); } //0 - ìàññèâ ðàñïðåäåëåíèÿ ïîð, 1 - ìàññèâ ñðåäíèõ ðàçìåðîâ ïîð, 2, 3 - ëåâàÿ è ïðàâàÿ ãðàíèöû, 4 - ñðåäíèé ðàçìåð ïîð, 5 - ìàêñèìàëüíûé
...
for (k=v0-1; k<qg; k++) { //èäåì ïî prgr01, legr01
    f=1; x=-1;
        for (q=p; q<n1; q++) { //èäåì ïî prgr0
