...
double **napMasEKTPVerNac(double, double, double, double, int, int, int, int, int, int, int, int, char *, int, double, double, double, double *, int, double *, double *, int, double *, double *, int, int, double *, double *, double *, int, double);
...
{
	int cemdu=6, k=1, nk=0, j=0, jk=0, f=cemdu, qn=0, cemv=cem, vybmar=0;
	double *stchsrver=NULL, *ektpv=NULL, *kttkv=NULL;
	...
	stchsrver=new double[cemv]; if (!stchsrver) { cout << snmv << endl; k=getchar(); exit(1); } for (k=0; k<cemv; k++) stchsrver[k]=hf; 
	...
	kttkv=opredKTPTverKarkVerm(etev, ektpv, porver, wsio, walo, wmgo, vyfv, vysv, vpkf, isrp, kuscv, tkuscv, dmkooscv, stchsrver, 
		cemv, fl, ktpvoz, tevoz, Prvoz, dmkvoz, snmv);
	...
	k=0; po[k]=nf; mu[k]=etev; k++; mu[k]=ektpv; k++; mu[k]=tgorv; k++; 
	mu[k]=tholv; k++; mu[k]=qobv; k++; mu[k]=po; k++; mu[k]=kttkv; k++; mu[k]=stchsrver; 
	return mu;
}
