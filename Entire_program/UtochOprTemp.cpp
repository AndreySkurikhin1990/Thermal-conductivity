	...
	double tol, char *snm)
{
	...
	k=0; teks[k]=kx*xi+bx;
	for (k=1; k<ksu; k++) { if (k%2) xi=xi+htch; else xi=xi+hvozd; //массив середин каждой из стенок по толщине
	teks[k]=kx*xi+bx; }
	return teks;
}
	...
	double hvo, double ptpob, int kost, char *snm, double zn)
{
	...
	for (k=1; k<n; k++)  { 
		...
		if (lamt<eps) lamt=0.0; //ищем КТП твердого скелета при данной температуре
		lamvo=opredKTPTKToch(ktpvo, vte, tt, dmv); 
		if (lamvo<eps) lamvo=0.0; //ищем КТП воздуха при данной температуре
		if ((fabs(lamt)<eps) || (fabs(lamvo)<eps)) r=0.0; 
		else { if (k%2) r=htk/lamt; else r=hvo/lamvo; r=r*zn*ptpob; }
		tt=tp+r; tp=tt; //рассчитываем температуру на границах стенок 
	...
	double hvo, double ptpob, int kost, char *snm, double zn, double *dpctp)
{
	...
	for (k=1; k<n; k++)  { 
		lamt=opredKTPTKToch(ktptk, ete, tt, ce); if (lamt<eps) lamt=0.0; //ищем КТП твердого скелета при данной температуре
		lamvo=opredKTPTKToch(ktpvo, vte, tt, dmv); if (lamvo<eps) lamvo=0.0; //ищем КТП воздуха при данной температуре
		dpctpe=opredKTPTKToch(dpctp, ete, tt, ce); if (dpctpe<eps) dpctpe=0.0; //ищем относительное сечение перемычки при данной температуре
		if ((fabs(lamt)<eps) || (fabs(lamvo)<eps)) r=0.0; 
	...
	double zn, char *snm, double Tnach, double *Tpctve)
{
	...
	for (k=0; k<n; k++)  { 
		...
		Tpctve[k]=tx; ht=ht+ho; } //рассчитываем температуру на границах стенок твердого каркаса //cout << endl; 
	return Tpctve;
}