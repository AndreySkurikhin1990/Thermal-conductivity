...
using namespace std;
//-----
double *opredKTPTverKarkSha(double *, double *, double, double, double, double, int, double *, double *, double *, int, int, 
	int, double *, double *, double *, int, char *);
...
{
	int k=0, j=0, m=3, w=4, q=3, vsma=5, vsmi=0;
	...
	if (vrsh==1) { sfeosha=1.21*ko; smgosha=0.29*ko; salosha=42.6*ko; ssiosha=hf-salosha-sfeosha-smgosha; porsha=11.0144*ko; } //ШПД-41
	if (vrsh==2) { sfeosha=1.64*ko; smgosha=0.36*ko; salosha=35.9*ko; ssiosha=59.1e-2; porsha=25.2*ko; } //ШБ-1 2-1
	if (vrsh==3) { sfeosha=1.66*ko; smgosha=0.4*ko; salosha=37.3*ko; ssiosha=57.4*ko; porsha=26.5*ko; } //ШВ-1 1-1
	if (vrsh==4) { sfeosha=1.24*ko; smgosha=0.29*ko; salosha=41.9*ko; ssiosha=54*ko; porsha=11.5*ko; } //ШПД
	if (vrsh==5) { sfeosha=1.54*ko; smgosha=0.3*ko; salosha=38.6*ko; ssiosha=56.5*ko; porsha=16.5*ko; } //ШКУ-32 3-1
	if ((vrsh<vsmi) && (vrsh>vsma)) { cout << "Nepravilno ukazan nomer!"; k=getchar(); exit(1); }
...
double **napMasEKTPShaNac(double wmg, double wsi, double wal, double porsha, int vybves, int vybsha, int vystsha, int vrsh, 
	char *snms, double y0, int dmkos, double tnacs, double detes, double tnoscs, double dtoscs, double *etesha, int cem, 
	double *tkuscs, double *kuscs, int dmkooscs, double *dkoscst, double *dkoscsm, int dkoscsl, double *ktpvozsha, double *temvozsha, 
	double *Prvozsha, int dmvozsha, double dko)
{
	int cemdu=6, k=0, j=0, f=cemdu, cems=cem, vfv=0, vsv=0, vpkf=0, isrp=0, fl=0, vybmar=0;
	...
	double g=0.0, s=0.0, nf=0.0, tn=0.0, **muv=new double*[f], *ektpsha=new double[cems];
	if ((!kektps) || (!ektpsha)) { cout << snms << endl; k=getchar(); exit(1); }
	porsha=NovNapMas(vybves, vybsha, k, k, k, k, k, k); cout << "por = " << porsha << "\t";
	...
	vsm=mu[k]; j=0; vs=vsm[j]; j++; vss=vsm[j]; j++; porsha=vsm[j]; 
	...
	//-----
	for (k=0; k<cems; k++) cout << "tem = " << etesha[k] << "\t"; cout << endl;
	for (k=0; k<dmkos; k++) cout << "ko ( " << k << " ) = " << kektps[k] << "\t"; cout << endl;
	...
	for (j=0; j<dmkos; j++) { s=s+kektps[j]*pow(t, g); g=g+hf; } 
	ektpsha[k]=s; } 
	...
	double *kttks=opredKTPTverKarkNach(etesha, ektpsha, porsha, wsi, wal, wmg, vfv, vsv, vpkf, isrp, kuscs, tkuscs, dmkooscs,
		stchsrsha, cems, fl, ktpvozsha, temvozsha, Prvozsha, dmvozsha, snms, vybves, vrsh, vystsha, vybmar);
	//-----
