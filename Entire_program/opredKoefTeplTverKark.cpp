...
#include <iostream>
using namespace std;
const double pi = acos(-1e0), porex=95.0*1e-2, tocras=1e-8, pormakvi=53.0*1e-2;
const double porist=28.0*1e-2, tem0=273.15, pgl=1e5, lgl=1e-9;
const int dsnftk=50, hko=1000, dmvtk=28, dmsrps=23;
...
double ReshNeyaUravMaxKoTepl(double, double, double); //Ìàêñèìàëüíûé
double ReshNeyaUravMinKoTepl(double, double, double); //Ìèíèìàëüíûé
double ReshNeyaUravKoTeplLiechtenecker(double, double, double); //Ëèõòåíýêåð
double opredRdeltaSrVzaPro(double, double, double, int); //Âçàèìîïðîíèêàþùèå êîìïîíåíòû
double KoTeplFrickEllipsoidChasti(double, double, double); //Ôðèê - ýëëèïñîèäû
double ReshNeyaUravKoTeplMaxwell(double, double, double); //Ìàêñâåëë
double ReshNeyaUravKoTeplBurgFrik(double, double , double); //Áóðãåð-Ôðèê
double ReshNeyaUravKoTeplShumanVoss(double, double, double); //Øóìàí-Âîññ
double ReshNeyaUravKoTeplBruggman(double, double, double); //Áðóããìàí
double ReshNeyaUravKoTeplGoringChirchillSpher(double, double, double); //Ãîððèíã-×åð÷èëëü (ñôåðû)
double ReshNeyaUravKoTeplGoringChirchillCyl(double, double, double); //Ãîððèíã-×åð÷èëëü (öèëèíäðû)
double ReshNeyaUravKoTeplGoringChirchillCylPlotSl(double, double, double, double); //Ãîððèíã-×åð÷èëëü (ïëîòíûé ñëîé)
double ReshNeyaUravKoTeplMaxwAcken(double, double, double); //Ìàêñâåëë-Ýéêåí
double ReshNeyaUravKoTeplSwift(double, double); //Ñâèôò
double MetodHashinaShtrikmanaMin(double, double, double); //Õàøèí-Øòðèêìàí (ìèíèìàëüíûé)
double MetodHashinaShtrikmanaMax(double, double, double); //Õàøèí-Øòðèêìàí (ìàêñèìàëüíûé)
...
double opredDulnLam1Sha(double, double, double, double, double, double);
...
double *oprKTPTverKarkVermi(double *, double *, double, double, double, double, int, int, double, double, int, double *, double *, double *, int, int, double *, double *, int, double);
...
double opredDulnLam1Ver(double, double, double, double, double, double);
...
sntk[k]='D'; k++; sntk[k]=':'; k++; sntk[k]='\\'; k++; sntk[k]='\\'; k++; sntk[k]='_';  k++; sntk[k]='À';  k++;
sntk[k]='ñ'; k++; sntk[k]='ï'; k++; sntk[k]='è';  k++; sntk[k]='ð';  k++; sntk[k]='à';  k++; sntk[k]='í';  k++;
sntk[k]='ò'; k++; sntk[k]='ó'; k++; sntk[k]='ð';  k++; sntk[k]='à';  k++; sntk[k]='\\'; k++; sntk[k]='\\'; k++;
sntk[k]='t'; k++; sntk[k]='m'; k++; sntk[k]='p';  k++; sntk[k]='\\'; k++; sntk[k]='\\'; k++; sntk[k]='\0';
k=0;
sstk[k]='C';  k++; sstk[k]=':';  k++; sstk[k]='\\'; k++; sstk[k]='\\'; k++; sstk[k]='U';  k++; sstk[k]='s';  k++;
sstk[k]='e';  k++; sstk[k]='r';  k++; sstk[k]='s';  k++; sstk[k]='\\'; k++; sstk[k]='\\'; k++; sstk[k]='À';  k++;
sstk[k]='í';  k++; sstk[k]='ä';  k++; sstk[k]='ð';  k++; sstk[k]='å';  k++; sstk[k]='é';  k++; sstk[k]='\\'; k++;
...
sstk[k]='_';  k++; sstk[k]='À';  k++; sstk[k]='ñ';  k++; sstk[k]='ï';  k++; sstk[k]='è';  k++; sstk[k]='ð';  k++;
sstk[k]='à';  k++; sstk[k]='í';  k++; sstk[k]='ò';  k++; sstk[k]='ó';  k++; sstk[k]='ð';  k++; sstk[k]='à';  k++;
...
return srzn; }
double opredDulnLam1Sha(double po, double T, double eps, double lavo, double lae, double srch)
{ double ladb=1e5, lada=1e-5, ra=1e2, ep=1e-7, ladc, fa, fb, fc; int h=0, kit=100;
while ((ra>ep) && (h<kit)) {
ladc=(lada+ladb)/2;    
fa=DulnevSloForSha(po,T,eps,lavo,lada,srch); fa=fa-lae;
fb=DulnevSloForSha(po,T,eps,lavo,ladb,srch); fb=fb-lae;
fc=DulnevSloForSha(po,T,eps,lavo,ladc,srch); fc=fc-lae;
if ((fc*fb>0.0) && (fa*fc<0.0)) ladb=ladc; if ((fc*fa>0) && (fb*fc<0)) lada=ladc; 
ra=fabs(fa-fb); h++;}
return ladc; }
...
double **mu=rasPorpoRazVer(por, vfv, k, vysove, isrp, vpkf); //0 - ñòàðûå, 1 - íîâûå çíà÷åíèÿ
...
double *ktptsv=oprKTPTverKarkVermi(tem, laefm, por, wsio, walo, wmgo, vfv, m, tena, dtos, dmkusc, kuscv, tkuscv, stchv, vysove, vpkf, srra, raspr, dmi, srp);
...
double *oprKTPTverKarkVermi(double *tem, double *laefm, double por, double wsio, double walo, double wmgo, int vfv, int n, double tena, double dtos, int dmkusc, double *kuscv, double *tkuscv, double *stchv, int vysove, int vpkf, double *srra, double *raspr, int dmv, double srp)
{ 
int k, cvp=22, cve=cvp+7*2, cved=cve+4, na=0, j=0, q=0, *no=new int[cved], m=dmsrpv, f=(cve-cvp)/2, vv=0, u=n, w=0;
double *srk=NULL, **sr=new double*[cved], *lavo=new double[n], ep=tocras;
...
if (!vfv) srch=(2e0+7e-1)/2e0; 
else if (vfv==1) srch=(8e0+4e0)/2e0; 
else if (vfv==2) srch=(1.6+0.35)/2e0; srch=srch*ko;
...
srk=new double[n]; if (!srk) { cout << "No memory!"; j=getchar(); exit(1); } for (j=0; j<n; j++) srk[j]=0.0;
	...
	else if ((k<cved) && (k>=cve)) srk=DopFunctOpredKTPTverChas(srk, tem, n, por, laefm, k-cve, vfv, lavo, srch); //if ((k==30) || (k==35)) for (j=0; j<n; j++) cout << "j = " << j << "\tk = " << k << "\tsrk = " << srk[j] << endl;
	q=1; for (j=0; j<n; j++) if ((srk[j]<lvmi) && (q>0)) q=-1; 
	if (q>0) { sr[w]=srk; no[w]=k; w++; p=p+ht; } else delete[]srk; } 
	cved=w; for (j=0; j<n; j++) { 
		...
		} } //cout << "cved = " << cved << "\tl = " << lvmi << "\tp = " << p << endl;
...
return srzn; }
//Ôóíêöèè ðàñ÷åòà ÊÒÏ òâåðäîãî êàðêàñà è âñïîìîãàòåëüíûå äëÿ íèõ
...
double ReshNeyaUravMaxKoTepl(double lae, double lan, double po) //lan - ÊÒÏ íåïðåðûâíîé ôàçû (âîçäóõ), èùåì ÊÒÏ òâåðäîãî ñêåëåòà
...
return ladc; }
double ReshNeyaUravKoTeplBruggman(double lae, double lan, double po)//Ìîäåëü Áðóããìàíà-Õàíàè - òîëüêî äëÿ êðóãëûõ ÷àñòèö
...
fa=lada+po*(lan-lada)*sa/3.0-lae; //îáúåìíàÿ äîëÿ ðàñòâîðåííîãî âåùåñòâà (ýëëèïñîèäîâ)
...
{ double L=1e0, Pa=1e1, Pdelt=1e2, delb=PoiskDelta(por,L), a=delb/Pa; //12, 13 - ãðóáàÿ è òî÷íàÿ îöåíêè ñîîò-íî
double la1, delm, lm, fb, la1u, epf=1e-2, fbn=0.0, fbnn=0.0, ep=1e-3, x, y;
la1=raschTeplVzaimoPronKomp(delb,L,laef,lavo); //ïîèñê ÊÒÏ  òâåðäîãî êàðêàñà - ãðóáàÿ îöåíêà
delm=L/Pdelt; //äåëüòà ìàëàÿ - òîëùèíà òðåùèí
lm=2.0*L-delm; //ýëü ìàëàÿ
...
if (fabs(fbn-fbnn)<epf) fb=(fbn+fbnn)/2.0; else if (fbn>fbnn) fb=fbnn; else fb=fbn; //lam2 - ïîðà, lam1 - òâåðäûé êàðêàñ //cout << "fb = " << fb << "\tx = " << x << "\ty = " << y << endl;
la1u=utochLamTverKark(delm,a,fb,lm,la1,delb,lavo,laef,L); //óòî÷íåíèå ÊÒÏ òâåðäîãî êàðêàñà - òîíêàÿ îöåíêà
if (vyb<13) { //cout << "lam_grub_otsen = " << la1 << endl; 
return la1; } else { //cout << "lam_tonk_ots = " << la1u << endl; 
return la1u; } }
double PoiskDelta(double m2, double L) //ïîèñê äåëüòû áîëüøîé
{ double ca=0.0, cb=1e0, cc, ra=fabs(ca-cb), ep=tocras, fa, fb, fc; int h=0, nit=hko; //cc - îòíîñèòåëüíûé ðàçìåð áðóñà
while ((ra>ep) && (h<nit)) {
cc=(ca+cb)/2.0;
fa=2.0*pow(ca,3.0)-3.0*pow(ca,2.0)+1.0-m2; 
fb=2.0*pow(cb,3.0)-3.0*pow(cb,2.0)+1.0-m2; 
...
double raschTeplVzaimoPronKomp(double del, double L, double laef, double la_v) //âçàèìïîïðîíèêàþùèå êîìïîíåíòû
...
ra=fabs(la1a-la1b); h++; }
return la1c; }
double utochLamTverKark(double delm, double a, double phib, double lm, double la1, double delb, double lavo, double laef, double L) //ñòðóêòóðà ñ ïåðåìåííûì ñå÷åíèåì êîìïîíåíò
...
double R5a, R5b, R5c, Rdsa, Rdsb, Rdsc, Rdelmina, Rdelmaxa, Rdelminb, Rdelmaxb, Rdelminc, Rdelmaxc; //laz - ÊÒÏ êîìïîíåíòû, çàïîëíÿþùåé òðåùèíû (âîçäóõ)
...
while ((ra>ep) && (h<nit)) {
la1c=(la1a+la1b)/2e0;
lamsha=la1a; //òåðìîñîïðîòèâëåíèå øåéêè
R1a=delm/2.0/pow(a,2e0)/lamsha; 
Rp0a=lm/la1a/pi/pow(b,2e0); //ïîëíîå ñîïðîòèâëåíèå êâàäðàòíîãî áðóñà
R7a=2e0*Rp0a*phim;
lamshb=la1b; //òåðìîñîïðîòèâëåíèå øåéêè
R1b=delm/2e0/pow(a,2e0)/lamshb;
Rp0b=lm/la1b/pi/pow(b,2e0); //ïîëíîå ñîïðîòèâëåíèå êâàäðàòíîãî áðóñà
R7b=2e0*Rp0b*phim;
lamshc=la1c; //òåðìîñîïðîòèâëåíèå øåéêè
R1c=delm/2e0/pow(a,2e0)/lamshc;
Rp0c=lm/la1c/pi/pow(b,2e0); //ïîëíîå ñîïðîòèâëåíèå êâàäðàòíîãî áðóñà
R7c=2e0*Rp0c*phim;
R2a=2e0*Rp0a*phib; //òåïëîâîå ñîïðîòèâëåíèå âîñüìîé ÷àñòè öåíòðàëüíîãî áðóñà
R2b=2e0*Rp0b*phib;
R2c=2e0*Rp0c*phib;
R5a=1e0/(L-delb)/la1a;
R5b=1e0/(L-delb)/la1b;
...
return ladc; }
double MetodHashinaShtrikmanaMin(double laef, double lan, double por) //îöåíêà âèëêè - äëÿ èçîòðîïíûõ â ñðåäíåì îáðàçöîâ
...
return ladc; }
double *opredTvChaKoeTepSrInSpSha(int vyb, double po, double *tem, double *laefm, int lear, double *srla, double *lavoz, double srch) //îïðåäåëåíèå ÊÒÏ òâåðäîé ÷àñòè èíûìè ñïîñîáàìè
...
if (vyb == 1) lam=ReshNeyaUravMaxKoTepl(laef, lavo, po); //ìàêñèìàëüíûé ÊÒÏ - ñëîè èäóò ïàðàëëåëüíî òåïëîâîìó ïîòîêó
if (vyb == 2) lam=ReshNeyaUravMinKoTepl(laef, lavo, po); //ìèíèìàëüíûé ÊÒÏ - ñëîè èäóò ïåðïåíäèêóëÿðíî òåïëîâîìó ïîòîêó
...
double *DulnevKoefTepShamot(int vyb, double por, double *tem, int n, double *srk, double *laefm, double *lavo, double srp, int koel, double *srra, double *rpr) //ïðîãðàììà îïðåäåëÿåò ÊÒÏ òâåðäîãî êàðêàñà øàìîòà ìåòîäîì Äóëüíåâà
{ int k=0; //cout << "\tvyb = " << vyb << "\tkoel = " << koel; 
if (vyb<koel) return BezUchetaRaspedPorSha(por, tem, laefm, lavo, vyb, n, srk, srp); //1 - Êóáû, àäèàá., 2 - Êóáû, èçîò., 3 - Ïî Îäîëåâñêîìó 1, 4 - Ïî Îäîëåâñêîìó 2, 5 - Öèëèíäðû, àäèàá., 6 - Öèëèíäðû, èçîò.
else return UchetRapsredPorPoRazmSha(por, tem, laefm, lavo, vyb, n, srk, k, srra, rpr); //7 - Êóáû, àäèàá., 8 - Êóáû, èçîòåðì., 9 - Öèëèíäðû, àäèàá., 10 - Öèëèíäðû, èçîò., 11 - Ïî Îäîëåâñêîìó 1, 12 - Ïî Îäîëåâñêîìó 2
} 
double *DulnevKoefTepVerm(int vyb, double por, double *tem, int n, double *srk, double *laefm, double *lavo, double srp, int koel, double *srra, double *raspr, int vybves, int vpv, int dmi, int nac) //îïðåäåëÿåò ÊÒÏ òâåðäîãî êàðêàñà ìåòîäîì Äóëüíåâà
{ int k=0; double *ukaz=NULL; //cout << "\tvyb = " << vyb << "\tkoel = " << koel; 
if ((vyb-nac)<koel) ukaz=BezUchetaRaspedPorSha(por, tem, laefm, lavo, vyb-nac, n, srk, srp); //1 - Êóáû, àäèàá., 2 - Êóáû, èçîò., 3, 4 - Ïî Îäîëåâñêîìó 1, 2, 5 - Öèëèíäðû, àäèàá., 6 - Öèëèíäðû, èçîò.
else ukaz=UchetRapsredPorPoRazmVer(por, tem, laefm, lavo, vyb-nac, n, srk, k, srra, raspr, vybves, vpv, dmi); //7 - Êóáû, àäèàá., 8 - Êóáû, èçîòåðì., 9 - Öèëèíäðû, àäèàá., 10 - Öèëèíäðû, èçîò., 11 - Ïî Îäîëåâñêîìó 1, 12 - Ïî Îäîëåâñêîìó 2
return ukaz; } 
double *BezUchetaRaspedPorSha(double por, double *tem, double *lameff, double *lamvoz, int vy, int n, double *lamvy, double srp) //áåç ó÷åòà ðàñïðåäåëåíèÿ ïîð
...
lamadi[k]=DulKoefTep1Adi(lavo, por, laef); //àäèàáàòíîå ðàçáèåíèå, ïîëîñòü - ïðÿìîóãîëüíûé ïàðàëëåëåïèïåä
lamizo[k]=DulKoefTep1Izoterm(lavo, por, laef); //èçîòåðìè÷åñêîå ðàçáèåíèå, ïîëîñòü - ïðÿìîóãîëüíûé ïàðàëëåëåïèïåä
lamodom[k]=DulKoefTep1OdolevMatr(lavo, por, laef); //ïî Îäîëåâñêîìó (ìàòðèöû)
lamodos[k]=DulKoefTep1OdolevStatSm(lavo, por, laef); //ïî Îäîëåâñêîìó (ñòàò. ñìåñü)
lamcyladi[k]=DulKoefTep1AdiCyl(lavo, por, laef, lb); //àäèàáàòíîå ðàçáèåíèå, ïîëîñòü - öèëèíäð
lamcylizo[k]=DulKoefTep1IzotermCyl(lavo, por, laef, lb); //èçîòåðìè÷åñêîå ðàçáèåíèå, ïîëîñòü - öèëèíäð 
lamcylper[k]=DulKoefTep1CylPer(lavo, por, laef, lb); //ïîëîñòü - öèëèíäð, òåïëîâîé ïîòîê íàïðàâëåí ïåðïåíäèêóëÿðíî îñè
} 
for (k=0; k<n; k++) { laef=lameff[k]; lavo=lamvoz[k];
if (!vy) lamvy[k]=lamadi[k]; //êóá, àäèàá. //22
if (vy == 1) lamvy[k]=lamizo[k]; //êóá, èçîòåðì. //23
if (vy == 2) lamvy[k]=lamodom[k]; //ïî Îäîëåâñêîìó (ìàòðèöû) //24
if (vy == 3) lamvy[k]=lamodos[k]; //ïî Îäîëåâñêîìó (ñòàò. ñìåñü) //25
if (vy == 4) lamvy[k]=lamcyladi[k]; //öèëèíäð., àäèàá. //26
if (vy == 5) lamvy[k]=lamcylizo[k]; //öèëèíäð., èçîòåðì. //27
if (vy == 6) lamvy[k]=lamcylper[k]; //öèëèíäð, ïåðïåíäèêóëÿðíî òåïëîâîìó ïîòîêó //28
}
...
return lamvy; }
double *UchetRapsredPorPoRazmSha(double po, double *T, double *lameff, double *lamvoz, int vy, int n, double *lamvy, int no, double *srra, double *rpr) //ó÷åò ðàñïðåäåëåíèÿ ïîð
{ int k, m=dmsrps, j;
...
j=0; for (k=0; k<m; k++) { if (rpr[k]>ep) {
    pw[j]=rpr[k]*po/1e2; //îáúåìíàÿ äîëÿ ïîðû çàäàííîãî ðàçìåðà â ïîëíîì (âî âñåì) îáúåìå
    vo[j]=pow(srra[k],3.0)/pw[j]; //âñå ïîðû - êóáû
	mx=mx+vo[j]; j++; } }
lcub = pow(mx, 1.0/3.0); //îöåíêà ìàêñèìàëüíîãî çíà÷åíèÿ l - ðàçìåðà îáðàçöà
lb = lcub / pow(po, 1.0/3.0); //îöåíêà ðàçìåðà ïîðû
...
if (vy == 7) lamvy[k]=lamcubadia[k]; //êóá, àäèàá. //29
if (vy == 8) lamvy[k]=lamcubizo[k]; //êóá., èçîòåðì. //30
if (vy == 9) lamvy[k]=lamcyladi[k]; //öèë., àäèàá. //31
if (vy == 10) lamvy[k]=lamcylizo[k]; //öèë., èçîòåðì. //32
if (vy == 11) lamvy[k]=lamodo1[k]; //ïî Îäîëåâñêîìó (ìàòðèöû) //33
if (vy == 12) lamvy[k]=lamodo2[k]; //ïî Îäîëåâñêîìó (ñòàò. ñìåñü) //34
if (vy == 13) lamvy[k]=lamcylper[k]; //öèë., ïåðï. òåïë. ïîòîêó //35
...
return lamvy; }
double *UchetRapsredPorPoRazmVer(double po, double *T, double *lameff, double *lamvoz, int vy, int n, double *lamvy, int no, double *srra, double *raspr, int vybves, int pkv, int dmi) //ó÷åò ðàñïðåäåëåíèÿ ïîð
...
j=0; t=0.0; for (k=0; k<m; k++) { if (raspr[k]>ep) {
    pw[j]=raspr[k]*po/w; //îáúåìíàÿ äîëÿ ïîðû çàäàííîãî ðàçìåðà â ïîëíîì (âî âñåì) îáúåìå
    vo[j]=pow(srra[k], 3e0)/pw[j]; //âñå ïîðû - êóáû
	mx=mx+vo[j]; j++; t=t+ht; } } //if (vy==7) for (k=0; k<m; k++) cout << "raspr = " << raspr[k] << "\tsrra = " << srra[k] << "\t"; if (vy==7) for (k=0; k<m; k++) cout << "pw = " << pw[k] << "\tvo = " << vo[k] << "\t"; 
lcub = pow(mx, 1e0/3e0)/t; //îöåíêà ìàêñèìàëüíîãî çíà÷åíèÿ l - ðàçìåðà îáðàçöà
lb = lcub / pow(po, 1e0/3e0); //îöåíêà ðàçìåðà ïîðû //cout << "lcub = " << lcub << "\tlb = " << lb << "\tpo = " << po << endl;
...
if (vy == 7) lamvy[k]=lamcubadia[k]; //êóá, àäèàá. //29
if (vy == 8) lamvy[k]=lamcubizo[k]; //êóá., èçîòåðì. //30
if (vy == 9) lamvy[k]=lamodo1[k]; //ïî Îäîëåâñêîìó (ìàòðèöû) //31
if (vy == 10) lamvy[k]=lamodo2[k]; //ïî Îäîëåâñêîìó (ñòàò. ñìåñü) //32
if (vy == 11) lamvy[k]=lamcyladi[k]; //öèë., àäèàá. //33
if (vy == 12) lamvy[k]=lamcylizo[k]; //öèë., èçîòåðì. //34
if (vy == 13) lamvy[k]=lamcylper[k]; //öèë., ïåðï. òåïë. ïîòîêó //35
...
delete []lamcyladi; delete []lamcylizo; delete []lamcylper; delete []pw; delete []vo;
return lamvy; }
double DulKoefTep1Adi(double lam2, double m2, double lame) //lam1 - ÊÒÏ òâåðäîãî êàðêàñà, lam2 - ÊÒÏ âîçäóõà
...
k++; ra=fabs(lama-lamb); } //cout << "lamc = " << lamc << "\tfc = " << fc << "\tra = " << ra << "\tk = " << k << "\n"; 
return lamc; }
double DulKoefTep1OdolevMatr(double lam2, double m2, double lame) //ìàòðè÷íàÿ ãåòåðîãåííàÿ ñèñòåìà - îäíà ôàçà îáðàçóåò ñâÿçíóþ ìàòðèöó ïðè ëþáîé îáúåìíîé êîíöåíòðàöèè ýòîé ôàçû, ñèñòåìà èìååò âêëþ÷åíèÿ â âèäå êóáîâ, öåíòðû êîòîðûõ îáðàçóþò ïðîñòóþ êóáè÷åñêóþ ðåøåòêó, ðåáðà ïàðàëëåëüíû
{ double lamb=pgl, lama=lgl, ep=tocras, ra=1e4, lamc; int k=0, nit=hko; //lam2 - ÊÒÏ âîçäóõà
...
{ double lamb=pgl, lama=lgl, ep=tocras, ra=1e4, lamc; int k=0, nit=hko; //íåò ðåãóëÿðíîé ñòðóêòóðû, ñòàòèñòè÷åñêàÿ ñìåñü, ÷àñòèöû ðàñïðåäåëåíû õàîòè÷åñêè
double v1, v2, nua, nub, nuc, fa, fb, fc; //lam2 - ÊÒÏ âîçäóõà, èùåì ÊÒÏ òâåðäîãî ñêåëåòà
while ((ra>ep) && (k<nit)) {
lamc=(lama + lamb)/2.0;
v1=m2; v2=1.0-m2;
...
fa=nua+pow(pow(nua,2.0)+lama*lam2/2.0,0.5)-lame; //lam2 - ÊÒÏ â ïîðàõ
fb=nub+pow(pow(nub,2.0)+lamb*lam2/2.0,0.5)-lame; //lame - ÝÊÒÏ
...
{ double ra=1e0, lamb=pgl, lam1=lavo, lama=lgl, dt=1e0, h=1e0, Qo=laef*h*dt, rb=pow(por/pi,1e0/2e0)*lb; //lam1 - ÊÒÏ âîçäóõà, lam2 - ÊÒÏ ÒÒ
double lamc, bb, bm, int1, ab, ep=tocras, am, Q1, Q2, fa, fb, fc; //òåïëîâîé ïîòîê ïåðïåíäèêóëÿðåí îñè öèëèíäðà
int k=0, nit=hko; 
while ((ra>ep) && (k<nit)) {
    lamc=(lama+lamb)/2e0;
...
lam=opredDulnLam1Ver(por, ts, epsil, la_voz, la_e, srch); 
...
return srk; }
double opredDulnLam1Ver(double po, double T, double eps, double lavo, double lae, double srch)
...
{ double r=srch/2.0, d=srch, epf=1e-2; double Nk=pow(pow(m2,2.0)-10.0*m2+9.0,0.5); Nk=(m2+3.0+Nk)/2.0/m2; /*y2=3.3e-3*pow(1-m2,-2./9.); pud=0; hsl=30e-3; rona=0.2; ro1=rona/(1-m2); y2=y2*pow(pud+9.8*ro1*(1-m2)*hsl,1./3.); y1=1e-20; y2=y1;*/
...
{ double m20=porex /*îáùàÿ ïîðèñòîñòü*/, m2=m20-m22 /*m2 - ìåæçåðíîâàÿ ïîðèñòîñòü (âíåøíÿÿ), m22 - âíóòðåííÿ ïîðèñòîñòü*/;
double A=pow(74e-2/(1e0-m2),1e0/3e0); 
double gu=9.81, hv=3e1*1e-3, rov=125.0, sigb=2.156e5 /*ïðåäåë ïðî÷íîñòè ïðè ñæàòèè ìàòåðèàëà ÷àñòèö, Ïà*/, ka=7e0/5e0, T, ladc, ladb, lada; 
double H0=101325.0, H=1e5, kB=1.380658e-23, d=(6e-1*21e-2+65e-2*79e-2)*1e-10, sigse=pi*pow(d,2e0)/4e0, E=8.428e6 /*ìîäóëü Þíãà âåðìèêóëèòà*/, laef; 
double akk=9e-1 /*êîýôôèöèåíò àêêîìîäàöèè*/, ko1=0.2595/(1-0.2595), ko2=0.2595-ko1, fp=0.2595/m2-(ko1*m2+ko2), ep, uo, up=1e0, ra;
...
izco=2e0*(5.67e-2)*pow(stch[k],2e0)*3e0/4e0;
    lam0=opredTeploprovVozd(T); /*òåìïåðàòóðà â Ê*/
    n=H/kB/T;
    dliSvoPro=1e0/pow(2e0,1e0/2e0)/n/sigse; //äëèíà ñâîáîäíîãî ïðîáåãà ìîëåêóëû âîçäóõà
    cPr=opredPrVozd(T-tem0); //÷èñëî Ïðàíäòëÿ
    N=pow(pow(m2,2e0)-1e1*m2+9.0,0.5); N=(m2+3e0+N)/2e0/m2;
    debo=rov*gu*hv; /*óäåëüíàÿ íàãðóçêà*/
    ...
    sigm4=lamg4*pi*r*(pow(2e0,1e0/2e0)*A-1e0)/2e0; //áîëüøàÿ ïîðà - öèëèíäð
...
double *MetodDulnevaSigalovoyDopVer(double *tem, int lete, double m22, double *laefm, int vfv, double r) /*m22 - ïîðèñòîñòü âåðìèêóëèòà ôðàêöèè 2-0,7 ìì, èç ñòàòüè "Òåïëîïðîâîäíîñòü ìîíî- è ïîëèäèñïåðñíûõ ìàòåðèàëîâ"*/
{ double m20=porex /*îáùàÿ ïîðèñòîñòü*/, m2=m20-m22;
double A=pow(74e-2/(1e0-m2),1e0/3e0) /*alpha - ðàçìåð âîçäóøíîé îáîëî÷êè (îðåîëà)*/, gu=9.8, hv=30e-3, rov=125., sigb=2.156e5; //ïðåäåë ïðî÷íîñòè ïðè ñæàòèè ìàòåðèàëà ÷àñòèö, Ïà
double ka=7e0/5e0, H0=101325.0, H=1e5, kB=1.380658e-23, d=((6e-1)*(21e-2)+(65e-2)*(79e-2))*(1e-10), sigse=pi*pow(d,2.0)/4.0;
double E=8.428e6 /*ìîäóëü Þíãà âåðìèêóëèòà*/, akk=0.9 /*êîýôôèöèåíò àêêîìîäàöèè*/, fa=0.0, fb=0.0, fc=0.0, up=1.0, uo, izco;
...
ladc=(lada+ladb)/2e0; T=tem[k];
	...
	izco=2e0*(5.67e-2)*pow(stch[k],2e0)*3e0/4e0;
	lam0=opredTeploprovVozd(T); /*òåìïåðàòóðà â Ê*/
    n=H/kB/T; dliSvoPro=1e0/pow(2.0,0.5)/n/sigse; /*äëèíà ñâîáîäíîãî ïðîáåãà ìîëåêóëû âîçäóõà*/
    cPr=opredPrVozd(T-tem0); /*÷èñëî Ïðàíäòëÿ*/ N=pow(pow(m2,2e0)-10.0*m2+9e0,1e0/2e0); N=(m2+3e0+N)/2e0/m2;
    ...
    X=4.45*(A*log(A/fabs(A-1e0))-1e0)/(1e0+B/H/dem1); //X - ìîëåêóëÿðíûé ïåðåíîñ òåïëà ÷åðåç îðåîëû âîêðóã ÷àñòèöû
    Zs=2.23*(pow(2e0,1e0/2e0)*A-1e0)/(1e0+B/H/dem3); //Z, Z' - ïåðåíîñ òåïëà ÷åðåç ïîðû
    Zss=2.23/(1e0+B/H/dem3)/(pow(2e0,1e0/2e0)*A-1e0);
    V=(pow(A,2e0)-pi/2e0)/(1e0+B/H/dem5)/A; //V - ÷åðåç äîïîëíèòåëüíûå ïóòè (ñòîëáû) ïðè ïîðèñòîñòè áîëåå 75 %
	...
	ra=fabs(fa-fb); h++; }
ladi[k]=ladc; } return ladi; }
double *MetodDulnevaSigalovoyBezIspVer(double *tem, int lete, double m22, double *laefm, int vfv, double r) //m22 - ïîðèñòîñòü âåðìèêóëèòà
{ double m20 = porex /*îáùàÿ ïîðèñòîñòü*/, m2=m20-m22; //èç ñòàòüè "Òåïëîïðîâîäíîñòü çåðíèñòûõ ñèñòåì"
double A=pow(74e-2/(1e0-m2),1e0/3e0) /*alpha - ðàçìåð âîçäóøíîé îáîëî÷êè (îðåîëà)*/, gu=9.8, hv=30e-3, rov=125.0, sigb=2.156e5; //ïðåäåë ïðî÷íîñòè ïðè ñæàòèè ìàòåðèàëà ÷àñòèö, Ïà
double ka=7e0/5e0, H0=101325.0, H=1e5, kB=1.380658e-23, d=(6e-1*21e-2+65e-2*79e-2)*1e-10, sigse=pi*pow(d,2e0)/4e0;
double E=8.428e6 /*ìîäóëü Þíãà âåðìèêóëèòà*/, Gms=20.58e6 /*ìîäóëü ñäâèãà*/, akk=9e-1 /*êîýôôèöèåíò àêêîìîäàöèè*/;
...
{ double *te=arrTempAir(), *ro=arrPlotnAir(), r=0.0;
int f=1, p=0, k, n=dmvtk; for (k=0; k<n; k++) {
	if ((te[k]>T) && (f)) { p=k; f=0; }
if ((f) && (!p)) p=n-1; if ((!f) && (!p)) p=1;
if (p) r=ro[p-1]+(ro[p]-ro[p-1])*(T-te[p-1])/(te[p]-te[p-1]);
} return r; }
double *arrTempAir()
...
ar[k]=1.06; k++; ar[k]=1.029; k++; ar[k]=1.; k++; ar[k]=0.972; k++; ar[k]=0.946; k++; ar[k]=0.898; k++;
...
int h=0, nit=hko; //xc - îòíîñèòåëüíûé ðàçìåð áðóñà
while ((ra>ep) && (h<nit)) {
xc=(xa+xb)/2e0;
...
f=1; for (k=0; k<n; k++) if (fabs(ta[k]-laefm[k])<e) { f=-1; break; } 
if (f<0) for (k=0; k<n; k++) ta[k]=0.0;
return ta; }
