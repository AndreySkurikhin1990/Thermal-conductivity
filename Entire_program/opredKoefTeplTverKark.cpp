...
double *DulnevZern(double, double *, double *, int, double *, double *, double, double, double, double, int, int, double *, double *, double *, double *, double *, int, int);
...
double DulnevSloForVer(double, double, double, double, double, double, double);
...
double *DopFunctOpredKTPTverChas(double *, double *, int, double, double *, int, int, double *, double, double *, double *, double *, double *, int, double);
double **vybFunRasPorPoRazSha(double, int, int, char *);
...
double *oprKTPTverKarkShamota(double *, double *, double, double, double, double, int, int, double *, double *, double *, double *, double *, int, double, double *, double *, double *, int, int, char *);
double *opredKTPTverKarkVerm(double *, double *, double, double, double, double, int, int, int, int, double *, double *, int, double *, int, int, double *, double *, double *, int, char *);
double *oprKTPTverKarkVermi(double *, double *, double, double, double, double, int, int, int, double *, double *, double *, int, int, double *, double *, int, double, int, double *, double *, double *, int, int, char *);
...
double *oprKTPTverKarkitom(double *, double *, double, double, double, double, int, int, double *, double *, double *, int, double *, double *, int, int, int, double, double *, double *, double *, int, char *);
...
double *oprKTPTverKarkkvi(double *, double *, double, double, double, double, int, int, double *, double *, double *, double *, double *, int, int, int, double, double *, double *, double *, int, char *);
...
//-----//Управляющие функции
double *opredKTPTverKarkSha(double *tem, double *laefm, double por, double wsio, double walo, double wmgo, int dmkusc, double *kuscs, 
	double *tkuscs, double *stchs, int lentem, int vrsh, int vystsha, double *ktpvotk, double *vtetk, double *Prvo, int dmkv, char *snm)
{ 
	int u=lentem, k=0, dmi=0, l=k+1, q=l+1, v=q+1, w=v+1, i=w+1, vybves=0;
	double **mu=vybFunRasPorPoRazSha(por, vrsh, vystsha, snm);
	double *raspr=mu[k], *srra=mu[l], *legr=mu[q], *prgr=mu[v], *ms=mu[w], srp=ms[k];
	double *mm=mu[i], marp=mm[k], tocras=1e-8, t=tocras, ht=1e0; 
dmi=0; while (t<marp) { dmi++; t=t+ht; } cout << "dmi = " << dmi << endl;
double *ktptks=oprKTPTverKarkShamota(tem, laefm, por, wsio, walo, wmgo, u, dmkusc, kuscs, tkuscs, stchs, srra, raspr, dmi, srp, ktpvotk, vtetk, Prvo, dmkv, vybves, snm);
if (raspr) delete[]raspr; if (srra) delete[]srra; if (legr) delete[]legr; 
if (prgr) delete[]prgr; if (ms) delete[]ms; if (mm) delete[]mm; if (mu) delete[]mu; 
return ktptks; }
double *oprKTPTverKarkShamota(double *tem, double *laefm, double por, double wsio, double walo, double wmgo, int u, int dmkusc, 
	double *kuscs, double *tkuscs, double *stchs, double *srra, double *rpr, int dms, double srp, double *ktpvotk, double *vtetk, 
	double *Prvotk, int dmkv, int vybves, char *snm)
{ 
	int k=0, cvp=22, cve=cvp+7*2, cved=cve+1, f=0, na=0, j=0, q=0, *no=new int[cved];
	double ht=1e0, *srk=new double[u], **sr=new double*[cved], *lavo=new double[u];
	double tocras=1e-8, ep=tocras, *srzn=new double[u], y=0.0, pormax=5e-1, lvmi=ht, ot=ht/3e0;
if ((!no) || (!sr) || (!lavo) || (!srzn) || (!srk)) { cout << snm << endl; k=getchar(); exit(1);} for (k=0; k<cved; k++) no[k]=0;
double s=0.0, p=0.0, x0=fabs(srp)/pow(por,ot)/2e0; for (k=0; k<u; k++) lavo[k]=opredKTPTKToch(ktpvotk, vtetk, tem[k], dmkv);
for (k=na; k<cved; k++) { srk=new double[u]; if (!srk) { cout << snm << endl; j=getchar(); exit(1); } for (j=0; j<u; j++) srk[j]=0.0;
    if (k==na) srk=DulnevZern(por, tem, srk, na, laefm, lavo, wsio, walo, wmgo, x0, u, dmkusc, kuscs, tkuscs, stchs, Prvotk, vtetk, dmkv, vybves);
...
return srzn; }
double *opredKTPTverKarkVerm(double *tem, double *laefm, double por, double wsio, double walo, double wmgo, int vfv, int vysove, 
	int vpkf, int isrp, double *kuscv, double *tkuscv, int dmkusc, double *stchv, int lentem, int fl, double *ktpvotk, double *vtetk, 
	double *Prvoz, int dmkvoz, char *snm)
{
	int k=0, dmi=0, l=k+1, m=l+1, n=m+1, q=n+1, v=q+1, vybves=1; 
	double **mu=RasPorPoRazVer(por, vfv, vysove, isrp, vpkf, snm); //0 - старые, 1 - новые значения
	double *raspr=mu[k], *srra=mu[l], e=1e-6, koef=1e6, tocras=1e-8;
	double *prgr=mu[m], *legr=mu[n], *ms=mu[q], srp=0.0, *mm=mu[v], marp=0.0, t=tocras, ht=1e0;
k=0; srp=ms[k]; marp=mm[k]; if (marp>ht) koef=ht;
t=e; dmi=0; while (t<(marp*koef)) { dmi++; t=t+ht; } 
srp=oprSrRazPor(vybves, vfv, por, vfv, vysove, isrp, vpkf, vpkf, vpkf, snm);
marp=oprMaxRazPor(vybves, vfv, por, vfv, vysove, isrp, vpkf, vpkf, vpkf, snm); 
cout << "dmi = " << dmi << "\tsrp = " << srp << "\tmrp = " <<marp << endl;
double *ktptsv=oprKTPTverKarkVermi(tem, laefm, por, wsio, walo, wmgo, vfv, lentem, dmkusc, kuscv, tkuscv, stchv, vysove, vpkf, srra, raspr, 
	dmi, srp, fl, ktpvotk, vtetk, Prvoz, dmkvoz, vybves, snm);
if (raspr) delete[]raspr; if (srra) delete[]srra; if (legr) delete[]legr; 
if (prgr) delete[]prgr; if (mm) delete[]mm; if (ms) delete[]ms;
if (mu) delete[]mu; 
return ktptsv; }
double *oprKTPTverKarkVermi(double *tem, double *laefm, double por, double wsio, double walo, double wmgo, int vfv, int n, int dmkusc, 
	double *kuscv, double *tkuscv, double *stchv, int vysove, int vpkf, double *srra, double *raspr, int dmv, double srp, int fl, 
	double *ktpvotk, double *vtetk, double *Prvotk, int dmkv, int vybves, char *snm)
{ 
	int k=0, cvp=22, cve=cvp+7*2, cved=cve+4, na=0, j=0, q=0, *no=new int[cved]; 
	int dmsrpv=18, m=dmsrpv, f=(cve-cvp)/2, u=n, w=0, pf=0;
	double tocras=1e-8, *srk=NULL, **sr=new double*[cved], *lavo=new double[n], ep=tocras, rmi=0.0, rma=0.0;
	double *srzn=new double[n], s=0.0, p=0.0, srch=0.0, r=0.0, ht=1e0, lvmi=ht, ko=1e-3;
...
srk=new double[n]; if (!srk) { cout << snm << endl; j=getchar(); exit(1); } for (j=0; j<n; j++) srk[j]=0.0;
	if (k==na) srk=DulnevZern(por, tem, srk, na, laefm, lavo, wsio, walo, wmgo, srch, n, dmkusc, kuscv, tkuscv, stchv, Prvotk, vtetk, dmkv, vybves);
	else if ((k<cvp) && (k>na)) srk=opredTvChaKoeTepSrInSpSha(k, por, tem, laefm, n, srk, lavo, srch);
    else if ((k<cve) && (k>=cvp)) srk=DulnevKoefTepVerm(k, por, tem, n, srk, laefm, lavo, srp, f, srra, raspr, vybves, vfv, dmv, cvp); 
	else if ((k<cved) && (k>=cve)) srk=DopFunctOpredKTPTverChas(srk, tem, n, por, laefm, k-cve, vfv, lavo, srch, stchv, Prvotk, 
...
return srzn; }
double *opredKTPTverKarkitom(double *tem, double *laefm, double por, double wsio, double walo, double wmgo, int vybves, int vvi, 
	double *kusci, double *tkusci, int dmkusc, double *stchi, int lentem, double *ktpvotk, double *vtetk, double *Prvo, int dmkv, char *snm)
{ int u=lentem, k=0, dmi=0, l=k+1, m=l+1, n=m+1, q=n+1, v=q+1;
double **mu=RasPorPoRazitom(vvi, snm), *raspr=mu[k], *srra=mu[l], *legr=mu[m];
double *prgr=mu[n], *ms=mu[q], *mm=mu[v], srp=ms[k], marp=mm[k];
double *ktptki=NULL, e=1e-8, t=e, ht=1e0; 
dmi=0; t=e; k=0; while (t<marp) { t=t+ht; dmi++; } //cout << "dmi = " << dmi << endl;
...
ktptki=oprKTPTverKarkitom(tem, laefm, por, wsio, walo, wmgo, u, dmkusc, kusci, tkusci, stchi, vvi, srra, raspr, vybves, k, dmi, srp, ktpvotk, vtetk, Prvo, dmkv, snm);
if (raspr) delete[]raspr; if (srra) delete[]srra; if (legr) delete[]legr; 
if (prgr) delete[]prgr; if (ms) delete[]ms; if (mm) delete[]mm; if (mu) delete[]mu; //cout << "dm = " << dm << endl; 
return ktptki; }
double *oprKTPTverKarkitom(double *tem, double *laefm, double por, double wsio, double walo, double wmgo, int u, int dmkusc, 
	double *kusci, double *tkusci, double *stchi, int vvi, double *srra, double *raspr, int vybves, int vpv, int dmi, 
	double srp, double *ktpvotk, double *vtetk, double *Prvotk, int dmkv, char *snm)
{
	int k=0, cvp=1+17+4, cve=cvp+12+2, cved=cve+1, f=(cve-cvp)/2, na=0, j=0, q=0, *no=new int[cved], n=u;
	double tocras=1e-8, *srk=NULL, **sr=new double*[cved], *lavo=new double[u], ep=tocras, *srzn=new double[u];
	double y=0.0, pormax=83e-2, ht=1e0, lvmi=ht;
if ((!no) || (!sr) || (!lavo) || (!srzn)) { cout << snm << endl; k=getchar(); exit(1);} for (k=0; k<cved; k++) no[k]=0;
double s=0.0, p=0.0, x0=fabs(srp)/pow(por,1e0/3e0)/2e0;
for (k=0; k<n; k++) { lavo[k]=opredKTPTKToch(ktpvotk, vtetk, tem[k], dmkv); if (lvmi>lavo[k]) lvmi=lavo[k]; }
for (k=na; k<cved; k++) {
srk=new double[u]; if (!srk) { cout << snm << endl; j=getchar(); exit(1);}
for (j=na; j<u; j++) srk[j]=0.0;
	if (k==na) srk=DulnevZern(por, tem, srk, na, laefm, lavo, wsio, walo, wmgo, x0, u, dmkusc, kusci, tkusci, stchi, Prvotk, vtetk, dmkv, vybves);
    else if ((k<cvp) && (k>na)) { srk=opredTvChaKoeTepSrInSpSha(k, por, tem, laefm, u, srk, lavo, x0); }
...
return srzn; }
double *opredKTPTverKarkkvi(double *tem, double *laefm, double por, double wsio, double walo, double wmgo, int vybves, 
	int vmkvi, double *kusck, double *tkusck, int dmkusck, double *stchkvi, int lentem, double *ktpvotk, double *vtetk, 
	double *Prvoz, int dmkvoz, char *snm)
{
	int u=lentem, k=0, dmk=k, l=k+1, m=l+1, n=m+1, q=n+1, v=q+1;
	double **mu=RasPorPoRazkvi(vmkvi, snm);
	double *raspr=mu[k], *srra=mu[l], *legr=mu[m], *prgr=mu[n], *ms=mu[q], *mm=mu[v];
	double srp=ms[k], marp=mm[k], *ktptkkvi=NULL, e=1e-8, t=e, ht=1e0; 
dmk=0; t=e; while (t<marp) { t=t+ht; dmk++; } //cout << "dm = " << dm << endl;
if ((vmkvi>10) || (vmkvi<4)) { cout << "Net takoy marki keramovermikulita!" << endl; k=getchar(); exit(1); } 
ktptkkvi=oprKTPTverKarkkvi(tem, laefm, por, wsio, walo, wmgo, u, dmkusck, kusck, tkusck, stchkvi, srra, raspr, vybves, vmkvi, dmk, srp, ktpvotk, vtetk, Prvoz, dmkvoz, snm);
if (raspr) delete[]raspr; if (srra) delete[]srra; if (legr) delete[]legr; 
if (prgr) delete[]prgr; if (ms) delete[]ms; if (mm) delete[]mm;
return ktptkkvi; }
double *oprKTPTverKarkkvi(double *tem, double *laefm, double por, double wsio, double walo, double wmgo, int u, int dmkusc, 
	double *kusci, double *tkusci, double *stchk, double *srra, double *raspr, int vybves, int vpv, int dmi, double srp, 
	double *ktpvotk, double *vtetk, double *Prvotk, int dmkv, char *snm)
{ 
	int k=0, na=0, cvp=22, cve=cvp+7*2, cved=cve+1, f=(cve-cvp)/2, j=0, q=0, *no=new int[cved], w=0; 
	double tocras=1e-8, *srk=NULL, **sr=new double*[cved], *lavo=new double[u], ep=tocras;
	double *srzn=new double[u], y=0.0, ht=1e0, lvmi=ht;
if ((!no) || (!sr) || (!lavo) || (!srzn)) { cout << snm << endl; k=getchar(); exit(1); } for (k=0; k<cved; k++) no[k]=0;
double s=0.0, p=0.0, x0=fabs(srp)/pow(por,1e0/3e0)/2e0;
for (k=0; k<u; k++) { lavo[k]=opredKTPTKToch(ktpvotk, vtetk, tem[k], dmkv); if (lvmi>lavo[k]) lvmi=lavo[k]; }
for (k=0; k<cved; k++) {
srk=new double[u]; if (!srk) { cout << snm << endl; j=getchar(); exit(1);}
for (j=0; j<u; j++) srk[j]=0.0;
	if (!k) srk=DulnevZern(por, tem, srk, na, laefm, lavo, wsio, walo, wmgo, x0, u, dmkusc, kusci, tkusci, stchk, Prvotk, vtetk, dmkv, vybves);
...
cout  << "Nomera nenulevyh elementov" << endl; for (k=0; k<q; k++) cout << no[k]+1 << " "; cout << endl << "q = " << q << endl; 
for (k=0; k<cved; k++) { srk=sr[k]; delete []srk; } if (sr) delete []sr; 
if (lavo) delete []lavo; if (no) delete []no; //k=getchar();
return srzn; }
...
{ double hf=1e0; if (x>0.0) return hf; else if (x<0.0) return (-hf); else return 0.0; }
...
while ((ra>ep) && (k<nit)) {
	...
	k++; ra=fabs(lama-lamb); } 
return lamc; }
double *DulnevZern(double por, double *te, double *srk, int na, double *laefm, double *lavo, double wsio, double walo, double wmgo, double srch, int n, int dm, double *kuscv, double *tkuscv, double *stchv, double *Prvotk, double *tvtk, int dmpv, int vybves)
{ 
	int k=0, f=0; 
...
lam=opredDulnLam1(por, ts, epsil, la_voz, la_e, srch, vybves, Prvoz); 
srk[k]=lam; }
...
{ 
	double tocras=1e-8, hf=1e0, ladb=1e5, lada=tocras, ra=hf, ep=tocras, ladc=0.0;
...
ladc=(lada+ladb)/2e0;
if (vv>0) { 
	fa=DulnevSloForVer(po, T, eps, lavo, lada, srch, Prvoz);
	fb=DulnevSloForVer(po, T, eps, lavo, ladb, srch, Prvoz);
	fc=DulnevSloForVer(po, T, eps, lavo, ladc, srch, Prvoz); }
else if (!vv) {
...
fa=fa-lae; fb=fb-lae; fc=fc-lae;
if ((fc*fb>0.0) && (fa*fc<0.0)) ladb=ladc; 
if ((fc*fa>0.0) && (fb*fc<0.0)) lada=ladc; 
ra=fabs(fa-fb); h++; } 
return ladc; }
double DulnevSloForVer(double m2, double T, double eps, double lavo, double lam1, double srch, double Prvoz)
{ 
	double r=srch/2.0, d=srch, epf=1e-2, hf=1e0, ot=hf/3e0, ov=hf/2e0; 
	double Nk=pow(pow(m2,2.0)-10.0*m2+9.0,0.5); Nk=(m2+3.0+Nk)/2.0/m2; //y2=3.3e-3*pow(1-m2,-2./9.); pud=0; hsl=30e-3; rona=0.2; ro1=rona/(1-m2); y2=y2*pow(pud+9.8*ro1*(1-m2)*hsl,1./3.); y1=1e-20; y2=y1;
double eta=1e-4, y1=1e-3, y2=y1/pow(eta,0.5); //y1=(10+50)*1e-4/2;
double y3=2.0*pow(Nk-hf,0.5)/Nk, y4=y3/pow(hf-m2,ot); //hsh=2e-3/2;
double hsh=1e-3, fb=0.0, fbn=0.0, fbnn=0.0;
if (fabs(y3)>0.0) fbn=opredFiBolN(y2,y2/y3); if (fabs(lam1)>0.0) fbnn=opredFiBolNN(lavo/lam1,m2);
if (fbn>hf) if (fbnn<hf) fb=fbnn; else fb=0.0; else if (fbnn>hf) fb=fbn; else 
if (fabs(fbn-fbnn)<epf) fb=(fbn+fbnn)/2.0; else if (fbn>fbnn) fb=fbnn; else fb=fbn; 
double A=pow(y2,2.0)-pow(y1,2.0), F=pow(hf-pow(y2,2.0),ov);
double D=pow(hf-pow(y3,2.0),ov), E=pow(y4,2.0)-pow(y3,2.0);
double delsrsz=d*(hsh+hf/Nk), lamszm=MoleSostTeplVozd(T, delsrsz, lavo, Prvoz);
double epspr=eps/(2.0-eps), sig=5.668e-8;
double lamszl=4.0*epspr*sig*delsrsz*pow(T,3.0), lamsz=lamszm+lamszl;
double nusz=lamsz/lam1, w=(lavo/lamszm-nusz*D)/(hf-nusz);
...
else if (p==1) srk=MetodDulnevaSigalovoyDopVer(tem, n, por, laefm, vfv, r, stche, Prv, lamvoz, temvoz, dmkov, ep);
else if (p==2) srk=MetodDulnevaSigalovoyBezIspVer(tem, n, por, laefm, vfv, r, stche, Prv, lamvoz, temvoz, dmkov, ep); 
...
double *VasFrayObsh(double *laefm, double *lavo, double po, int n)
{
	...
	double pi=acos(-1e0), tocras=1e-8, hf=1e0, ot=hf/3e0, dt=2e0/3e0, ov=hf/2e0, L=hf;
	double x=Poiskkx((hf-po)/4.0), db=L*x;
	...
	double Pn=125.0*9.8*30e-3, Aa=hf, Ab=hf, Ac=hf, kk=(1.5+2e0)/2.0, kb=(2.2+2.9)/2e0;
...
/*double **opredSredKTPTK()
{
	int cemtk=11, k=1, f=cemtk, j=0, q=j, w=j, b=11, l=j, fl=1, na=j, *ceti=new int[f], u=j, v=7;
