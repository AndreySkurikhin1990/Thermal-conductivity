function [ npv ] = napMasEKTPkviNac(vybkvi, vybvykhmas, ys0, ete) %isrp=0; %выбор метода измерений для фракции 2-0,7 мм: 0 - нестационарный, 1 - стационарный %выбор фракции: 0 - фракция 2-0,7 мм, 1 - фракция 8-4 мм, 2 - фракция 1,6-0,35 мм %выбор укладки: 1 - плоскопараллельная, 2 - вертикальная %выбор состояния: 0 - исходное, 1 - после повторных измерений, 2 - после прокаливания при 1000 град °С
format longg; vybvesch=3; %cem=length(ete); dete=1e2; te0=273.15; tnac=2e2+te0; cem=11; ete=tnac:dete:tnac+(cem-1)*dete;
    ident=0; thol=napMasEKTP(vybkvi, ete, ident, ys0);
    ident=ident+1; tgor=napMasEKTP(vybkvi, ete, ident, ys0);
    ident=ident+1; qob=napMasEKTP(vybkvi, ete, ident, ys0);
    ident=ident+1; ektpv=napMasEKTP(vybkvi, ete, ident, ys0);
    ident=ident+1; ete=napMasEKTP(vybkvi, ete, ident, ys0);
    thol=thol;
    ete=ete;
    f=1;
    k=0; tholn=vydelPol(thol, tgor, qob, ektpv, ete, k, f, vybvesch, vybkvi);
    k=k+1; tgorn=vydelPol(thol, tgor, qob, ektpv, ete, k, f, vybvesch, vybkvi);
    k=k+1; qobn=vydelPol(thol, tgor, qob, ektpv, ete, k, f, vybvesch, vybkvi);
    k=k+1; ektpvn=vydelPol(thol, tgor, qob, ektpv, ete, k, f, vybvesch, vybkvi);
    k=k+1; eten=vydelPol(thol, tgor, qob, ektpv, ete, k, f, vybvesch, vybkvi);
    eten=eten;
    tholn=tholn;
    switch (vybvykhmas)
        case (0)
...
function [ oa ] = napMasEKTP(vybkvi, te, ident, h)
        k=0; koefc=oprkoefKTPiskhchaokvi(k, te, h, vybkvi);
        k=1; koefh=oprkoefKTPiskhchaokvi(k, te, h, vybkvi); 
        k=2; koefq=oprkoefKTPiskhchaokvi(k, te, h, vybkvi);
		k=3; koeft=oprkoefKTPiskhchaokvi(k, te, h, vybkvi);
        k=4; koefs=oprkoefKTPiskhchaokvi(k, te, h, vybkvi);
    switch (ident)
        case (0)
            vm=koefc;
        case (1)
...
function [ oa ] = oprkoefKTPiskhchaokvi(v, efte, h, vybkvi) 
koef=poisMasKoefkvi(vybkvi);
lentem=length(efte);
ktpv=zeros(1,lentem);
x=1; y=x+1;
for k=1:lentem
ts=efte(k); laef=koef(y)*ts+koef(x); ktpv(k)=laef;
end
        k=0; thvn=opredTempHolGor(ktpv, efte, h, k, efte, vybkvi);
        k=k+1; tgvn=opredTempHolGor(ktpv, efte, h, k, efte, vybkvi); 
		k=k+1; qovn=opredTempHolGor(ktpv, efte, h, k, efte, vybkvi);
        k=k+1; ktpvn=opredTempHolGor(ktpv, efte, h, k, efte, vybkvi);
        k=k+1; tsvn=opredTempHolGor(ktpv, efte, h, k, efte, vybkvi);
            f=-1;
            vybvesch=1;
    k=0; thvnn=vydelPol(thvn, tgvn, qovn, ktpvn, tsvn, k, f, vybvesch, vybkvi);
    k=k+1; tgvnn=vydelPol(thvn, tgvn, qovn, ktpvn, tsvn, k, f, vybvesch, vybkvi);
    k=k+1; qovnn=vydelPol(thvn, tgvn, qovn, ktpvn, tsvn, k, f, vybvesch, vybkvi);
    k=k+1; ktpvnn=vydelPol(thvn, tgvn, qovn, ktpvn, tsvn, k, f, vybvesch, vybkvi);
    k=k+1; tsvnn=vydelPol(thvn, tgvn, qovn, ktpvn, tsvn, k, f, vybvesch, vybkvi);
        thvnn=thvnn;
        tsvnn=tsvnn;
...
function [ ktpk ] = poisMasKoefkvi(v)
...
%-----
function [ vyma ] = PoiskZavVelTem(v, efte, h, f, vm)
		...
		for jvsv=1:nnvysv
			...
			if ((vyfrve==0) || (vyfrve==2))
				for qvmi=1:nnvmivmf
                    ...
                    fl=-1;
                    k=0; temvctn=vydelPol(temvct, temvht, tepvt, ktp, ts, k, fl, vybvesch, vyfrve);
                    k=k+1; temvhtn=vydelPol(temvct, temvht, tepvt, ktp, ts, k, fl, vybvesch, vyfrve);
                    k=k+1; tepvtn=vydelPol(temvct, temvht, tepvt, ktp, ts, k, fl, vybvesch, vyfrve);
                    k=k+1; ktpn=vydelPol(temvct, temvht, tepvt, ktp, ts, k, fl, vybvesch, vyfrve);
                    k=k+1; tsn=vydelPol(temvct, temvht, tepvt, ktp, ts, k, fl, vybvesch, vyfrve);
                    if (f>0)
                    fl=1;
                    end
            ...
            elseif (vyfrve==1)
				for qvuv=1:nnvyuv
                    ...
                    ident=ident+1; ts=napMasEKTPVermik(vyfrve, vysove, vymivmf, efte, ident, h, vyukve, vm); 
                    j=-1;
                    k=0; temvctn=vydelPol(temvct, temvht, tepvt, ktp, ts, k, j, vybvesch, vyfrve); 
                    k=k+1; temvhtn=vydelPol(temvct, temvht, tepvt, ktp, ts, k, j, vybvesch, vyfrve); 
                    k=k+1; tepvtn=vydelPol(temvct, temvht, tepvt, ktp, ts, k, j, vybvesch, vyfrve); 
                    k=k+1; ktpn=vydelPol(temvct, temvht, tepvt, ktp, ts, k, j, vybvesch, vyfrve); 
                    k=k+1; tsn=vydelPol(temvct, temvht, tepvt, ktp, ts, k, j, vybvesch, vyfrve); 
                    if (f>0)
                    fl=1;
                    end
                    ident=0; temcq=rasMas(efte, temvhtn, temvctn, ktpn, tepvtn, tsn, temvs, temhq, temcq, ktpq, tepv, nomer, vyfrve, vysove, vymivmf, vyukve, fl, ident);
...
%-----
function [ oa ] = opredTempHolGor(ektp, ete, h0, v, efte, vybkvi) %моделирование процесса теплообмена в образце %n - длина массива ektp, l - длина массивов temvs, qob, ktp, temvh, temvc, ni - длина efte, qob - плотность теплового потока, которую может создать лабораторная установка 
    ektp=ektp;
    ete=ete;
	hf=1e0; te0=273.15; tn=22.0+te0;
    f=1;
	k=2; qon=PoiskZavVelTem(k, efte, h0, f, vybkvi);
    f=-1;
	k=4; tena=PoiskZavVelTem(k, efte, h0, f, vybkvi);
    ...
    ktp=zeros(1,ni);
        ...
        temvs=temvs;
    vybvesch=1; j=-1; 
    k=0; temvcn=vydelPol(temvc, temvh, qob, ktp, temvs, k, j, vybvesch, vybkvi);
    k=k+1; temvhn=vydelPol(temvc, temvh, qob, ktp, temvs, k, j, vybvesch, vybkvi);
    k=k+1; qobn=vydelPol(temvc, temvh, qob, ktp, temvs, k, j, vybvesch, vybkvi);
    k=k+1; ktpn=vydelPol(temvc, temvh, qob, ktp, temvs, k, j, vybvesch, vybkvi);
    k=k+1; temvsn=vydelPol(temvc, temvh, qob, ktp, temvs, k, j, vybvesch, vybkvi);
        switch (v)
            case (0)
                vm=temvcn;
            case (1)
...
%-----
function [ oa ] = napMasEKTPVermik(vyfrve, vysove, vymeizvemafr, te, ident, h, vyukve, vybkvi)
	...
	if ((vyfrve==0) || (vyfrve==2)) %фракция 2-0,7 мм или фракция 1,6-0,35 мм
		...
		k=0; koefc=oprkoefKTPiskhchao(k, te, h, vybkvi);
        k=1; koefh=oprkoefKTPiskhchao(k, te, h, vybkvi); 
        k=2; koefq=oprkoefKTPiskhchao(k, te, h, vybkvi);
		k=3; koeft=oprkoefKTPiskhchao(k, te, h, vybkvi);
        k=4; koefs=oprkoefKTPiskhchao(k, te, h, vybkvi);
...
%-----
function [ oa ] = oprkoefKTPiskhchao(v, efte, h, vybkvi) %vmiv - выбор метода измерений %0 - установка Netzsch - нестационарный метод
        mt=arrTem_Netzsch();
		ktpv=arrKTP_Netzsch();
		k=0; thvn=opredTempHolGor(ktpv, mt, h, k, efte, vybkvi);
        k=k+1; tgvn=opredTempHolGor(ktpv, mt, h, k, efte, vybkvi); 
		k=k+1; qovn=opredTempHolGor(ktpv, mt, h, k, efte, vybkvi);
        k=k+1; ktpvn=opredTempHolGor(ktpv, mt, h, k, efte, vybkvi);
        k=k+1; tsvn=opredTempHolGor(ktpv, mt, h, k, efte, vybkvi);
            f=-1;
            vybvesch=1;
    k=0; thvnn=vydelPol(thvn, tgvn, qovn, ktpvn, tsvn, k, f, vybvesch, vybkvi);
    k=k+1; tgvnn=vydelPol(thvn, tgvn, qovn, ktpvn, tsvn, k, f, vybvesch, vybkvi);
