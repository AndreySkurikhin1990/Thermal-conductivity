...
switch (NO)
    ...
    case 28
        ...
        viewF = SS/C*(1.-((1./PI)*(acos(B/A)-0.5*(sqrt(A*A+4.*HH*HH)*acos(B/A/CC)+B*asin(1./CC))/HH)) ...
            +A/HH/4); %перед A/HH/4 должен быть минус
    case 29
        if(NARG~=1)
            [viewF] = warning_ii(NO);
            return;
    ...
    case 39
        ...
        viewF = (WW*atan(1e0/WW)+HH*atan(1e0/HH)-HW*atan(1e0/HW) ...
            +25e-2*(log(C1)+W2*log(C2)+H2*log(C3)))/pi/WW;
