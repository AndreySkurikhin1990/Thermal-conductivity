...
for w=1:n
    ...
    T3=temvs(w);
    Ts=(Tk+T0)/2
f=opredBetaPBetaMVer(T0,Tk,y0,ektp,T0,qob,T3,vybitom);
end
t=0;
end
...
function [ t ] = opredBetaPBetaMVer(T0,Tk,tol,koeftep,Tz,qo,T3,vybitom)
sig=5.67*1e-8;
...
dv=dlinyvoln();
knuSha=SredGrafItom();
vybitom=1; %0 - ИТОМ-440, 1 - ИТОМ-620, 2 - ИТОМ-860, 3 - ИТОМ-1000
...
s=mean(koal)
Tsr=(T0+Tk)/2
dko2=opredKTPTKTochVer(dko2m, dkoscit, Tsr);
...
reiz=resuIzlu(tolt,T0,Tk,koal,dv,knuSha/s,npp,tetatausz,bpl,bmi,tau00,npr,length(tetataus),Te,ksi,0)
Iplu=resuIzlu(tolt,T0,Tk,koal,dv,knuSha/s,npp,tetatausz,bpl,bmi,tau00,npr,length(tetataus),Te,ksi,1)
Imin=resuIzlu(tolt,T0,Tk,koal,dv,knuSha/s,npp,tetatausz,bpl,bmi,tau00,npr,length(tetataus),Te,ksi,2)
Nt=2; taus=0:tau00/Nt:tau00; n=length(taus); ksi=taus/alsr;
kna=(Tk-T0)/tau0/Tz; kb=T0/Tz;
for k=1:n
    tetataus(k)=kna*taus(k)+kb;
    tetataus4(k)=tetataus(k)^4;
...
function [ rezizl ] = resuIzlu(tol,T0,Tk,koal,dl,alfs,npp,temp,Iplus,Iminus,tau0,npr,Nt,Tet,ksi,vybor)
p=length(dl); 
sigma=5.67e-8;
