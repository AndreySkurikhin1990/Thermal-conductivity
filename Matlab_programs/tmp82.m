%рассеяние только для цилиндров
function [ t ] = tmp82()
format long g;
npp=Kramers_ver()-1; npp=npp';
na=26.8; ko=27; de=1e-1;
dv=1.4:de:ko;
dvr=na:de:ko;
q=1; np=0; 
for k=1:length(dv)
...
fidTr=fopen('Cylindry_Tr-2.txt','w'); fiddpp=fopen('Cylindry_dpp-2.txt','w'); 
fidTrs=fopen('Cylindry_Trs-2.txt','w'); fiddnra=fopen('Cylindry_dnra-2.txt','w'); 
fidrazdT=fopen('Cylindry_razdT-2.txt','w'); fidn1=fopen('Cylindry_nKBr-2.txt','w'); 
fiddv=fopen('Cylindry_dl_vo-2.txt','w'); np(q)=npp(length(npp)); 
nofr=4 %no - номер фракции
nomkon=1 %nom - номер концентрации
%q=VyvodTrs(dvr,no,nom,fidTrs);
for k=1:length(dv)
    n1(k)=OprPokPreKBr(dv(k));
end
n1=n1'; tr=0; np=np'; npma=max(np); npmi=min(np);
for k=1:length(np)
    trk(k)=nacha(np(k),dvr(k),nofr,nomkon,npmi,npma,fidTr,fiddpp,fidTrs,fiddnra,fidrazdT,fidn1,fiddv);
    tr(k)=trk(1);
end
fclose(fidTr); fclose(fiddpp); fclose(fiddnra); fclose(fidrazdT); fclose(fidTrs); fclose(fidn1);
t=tr;
end
...
function t = VyvodTrs(dvr,vyfr,vyko,fidTrs)
        ...
        TRs=TrRef(w-1)+(TrRef(w)-TrRef(w-1))*(dvr-dlv(w-1))/(dlv(w)-dlv(w-1));
        fprintf(fidTrs,'%0.15f\n',TRs);
        end
        end
t=0;
