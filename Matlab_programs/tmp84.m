%рассеяние только для шаров - ИТОМ
function [ t ] = tmp84()
npp=Kramers_n_Itom()-1;
na=2.9; ko=27; de=1e-1; ep=de/1e1;
dv=1.4:de:ko;
dvr=na:de:ko;
q=1; np=0;
for k=1:length(dv)
...
fidTr=fopen('Shary_Tr-itom-1.txt','w'); fiddpp=fopen('Shary_dpp-itom-1.txt','w'); 
fidTrs=fopen('Shary_Trs-itom-1.txt','w'); fiddnra=fopen('Shary_dnra-itom-1.txt','w'); 
fidrazdT=fopen('Shary_razdT-itom-1.txt','w'); fidn1=fopen('Shary_nKBr-itom-1.txt','w'); 
fiddv=fopen('Shary_dl_vo-itom-1.txt','w'); 
npma=max(np); npmi=min(np);
no=1; %номер фракции
nom=1; %номер концентрации
for k=1:length(np)
...
t=2*erfcinv(1e-4) %t=2*PoisKorn()
if (no==1)
ti=RasFra60(t,popr,dv,nom,npmi,npma,fidTr,fiddpp,fidTrs,fiddnra,fidrazdT,fidn1,fiddv); %менее 63 мкм
else
    ti=[0];
end
tm=ti;
...
TrRef=SredGrafRass(vyfr,vyko,xve); dlv=(1e6)*dlinavolny; numele=length(dlv)-1; na=1; dvko=dvr
n1=OprPokPreKBr(dvr)
vve=opredEffPar(vv,vyfr,vyko); dnre=0; q=1; eps=1e-2; chit=1e2; dnc=0; dpp=dpp'
while (dvr<=dvko)
...
function [ alsr ] = SredGrafRass(vyfr,vyko,tol)
%vyfr - выбор фракции: 1 - 60, 2 - 60-100, 3 - 100-150, 4 - 150-200; vyko - выбор массовой доли: 1 - 0,1 %, 2 - 0,2 %, 3 - 0,3 %
if (vyfr==1)
    if (vyko==1)
Tal=privedkEdiPropus(TrkoItom1());
    elseif (vyko==2)
Tal=privedkEdiPropus(TrkoItom2());
    end
alsre=opredAlphSred(Tal,tol);
else
    alsre=[0];
end
alsr=alsre;
end
function [ al ] = opredAlphSred(Tal,tol)
nTal=length(Tal);
for k=1:nTal 
    alsr(k)=-log(Tal(k))/tol;
end
al=alsr;
end
...
mkbr=[230.078, 231.006]; 
mv=[0.952, 1.189]; 
tol=[0.64, 0.64];
rokbr=2.75; rov=0.44; n=length(mv);
for k=1:n
vkbr(k)=mkbr(k)/(1e3*rokbr); %в см3
vv(k)=mv(k)/(1e3*rov); %в см3
...
function t = VyvodTr(dvr,nofr,noko,tol)
dlv=(1e6)*dlinavolny();
TrRef=SredGrafRass(nofr,noko,tol); 
for k=1:length(dvr)
    dvro=dvr(k);
