function [ nf ] = DulnevZernSha(wmg,wal,wsi,por,te,na,laefm)
disp('По методу Дульнева для зернистых систем для шамота');
te0=273.15; te=te'; uo=urovPod(por); up=1e0;
for k=1:length(te)
    kosc(k)=koefoslab(wmg, wsi, wal, te);
    ts=te(k);
    epsi(k)=epsisred(ts);
eps(k)=epsi(k)*kosc(k);
la_voz(k)=opredKTPVozd(ts);
la_e=laefm(k);
lam(k)=opredDulnLam1Sha(por,ts,eps,la_voz(k),la_e);
lam(k)=urovOtsechen(lam(k),la_e,uo); %определить точнее
lam(k) = urovPodder(lam(k),la_voz(k),up);
end
epsi=epsi'; kosc=kosc'; laefm=laefm'; lam=lam';
srk = lam;
f = 1;
for k=1:length(srk)
    if ((srk(k) == 0) && (f > 0))
...
function ol1 = opredDulnLam1Sha(po,T,eps,lavo,lae)
ladb=1e5; lada=1e-5; ra=1e2; ep=1e-7; h=0; la=0; kit=1e2;
while ((ra>ep) && (h<kit))
ladc=(lada+ladb)/2;    
fa=DulnevSloForSha(po,T,eps,lavo,lada);
fa=fa-lae;
fb=DulnevSloForSha(po,T,eps,lavo,ladb);
fb=fb-lae;
fc=DulnevSloForSha(po,T,eps,lavo,ladc);
fc=fc-lae;
...
function lam4 = DulnevSloForSha(m2,T,eps,lavo,lam1)
r=SreRazPorSha(); r=1e-6*r; d=2*r;
