function koefte = RaschTepPot(nv,tau,y0,tol,tem,T0,Tk,Tz,alsred,Ns,koeftep,Ref,tau0)
jv=-koeftep*(Tk-T0)/(y0*(1e-6));
jv=abs(real(jv));
cons=abs((Tk-T0)/(y0*1e-6));
koten=0;
kotek=5e-1;
sck=1e4;
sc=0;
dlin=abs(kotek-koten);
sigma=5.668e-8; 
stch=1-Ref;
mshi=1e-4;
...
ppi=denTherFlux(nv,tol,tem,Tk,alsred,Ns,tau0,stch,sigma,tau,R0,Rtau0);
ppi=abs(real(ppi))
while (dlin>mshi)
    kotet=(koten+kotek)/2;
rqa=-(jv-koten*cons-ppi);
rqb=-(jv-kotek*cons-ppi);
rqc=-(jv-kotet*cons-ppi);
if (rqb*rqc>0)
    kotek=kotet;
end
if (rqa*rqc>0)
