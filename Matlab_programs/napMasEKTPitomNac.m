function [ npv ] = napMasEKTPitomNac(vybitom, vybvykhmas, ys0, ete) %isrp=0; %выбор метода измерений для фракции 2-0,7 мм: 0 - нестационарный, 1 - стационарный %выбор фракции: 0 - фракция 2-0,7 мм, 1 - фракция 8-4 мм, 2 - фракция 1,6-0,35 мм %выбор укладки: 1 - плоскопараллельная, 2 - вертикальная %выбор состояния: 0 - исходное, 1 - после повторных измерений, 2 - после прокаливания при 1000 град °С
format longg; vybvesch=2; %cem=length(ete); dete=1e2; te0=273.15; tnac=2e2+te0; cem=11; ete=tnac:dete:tnac+(cem-1)*dete;
    ident=0; thol=napMasEKTP(vybitom, ete, ident, ys0, vybvesch);
    ident=ident+1; tgor=napMasEKTP(vybitom, ete, ident, ys0, vybvesch);
    ident=ident+1; qob=napMasEKTP(vybitom, ete, ident, ys0, vybvesch);
    ident=ident+1; ektpv=napMasEKTP(vybitom, ete, ident, ys0, vybvesch);
    ident=ident+1; ete=napMasEKTP(vybitom, ete, ident, ys0, vybvesch);
    thol=thol;
    ete=ete;
    f=1;
    k=0; tholn=vydelPol(thol, tgor, qob, ektpv, ete, k, f, vybvesch, vybitom);
    k=k+1; tgorn=vydelPol(thol, tgor, qob, ektpv, ete, k, f, vybvesch, vybitom);
    k=k+1; qobn=vydelPol(thol, tgor, qob, ektpv, ete, k, f, vybvesch, vybitom);
    k=k+1; ektpvn=vydelPol(thol, tgor, qob, ektpv, ete, k, f, vybvesch, vybitom);
    k=k+1; eten=vydelPol(thol, tgor, qob, ektpv, ete, k, f, vybvesch, vybitom);
    eten=eten;
    tholn=tholn;
    switch (vybvykhmas)
        case (0)
...
function [ oa ] = napMasEKTP(vybitom, te, ident, h, vybvesch)
        k=0; koefc=oprkoefKTPiskhchaoitom(k, te, h, vybitom, vybvesch);
        k=1; koefh=oprkoefKTPiskhchaoitom(k, te, h, vybitom, vybvesch); 
        k=2; koefq=oprkoefKTPiskhchaoitom(k, te, h, vybitom, vybvesch);
		k=3; koeft=oprkoefKTPiskhchaoitom(k, te, h, vybitom, vybvesch);
        k=4; koefs=oprkoefKTPiskhchaoitom(k, te, h, vybitom, vybvesch);
    switch (ident)
        case (0)
            vm=koefc;
        case (1)
...
function [ oa ] = oprkoefKTPiskhchaoitom(v, efte, h, vybitom, vybvesch) %vmiv - выбор метода измерений %0 - установка Netzsch - нестационарный метод
koef=poisMasKoefItom(vybitom);
lentem=length(efte);
for k=1:lentem
ts=efte(k); laef=koef(2)*ts+koef(1); ktpv(k)=laef;
end
        k=0; thvn=opredTempHolGor(ktpv, efte, h, k, efte, vybitom, vybvesch);
        k=k+1; tgvn=opredTempHolGor(ktpv, efte, h, k, efte, vybitom, vybvesch); 
		k=k+1; qovn=opredTempHolGor(ktpv, efte, h, k, efte, vybitom, vybvesch);
        k=k+1; ktpvn=opredTempHolGor(ktpv, efte, h, k, efte, vybitom, vybvesch);
        k=k+1; tsvn=opredTempHolGor(ktpv, efte, h, k, efte, vybitom, vybvesch);
            f=-1;
    k=0; thvnn=vydelPol(thvn, tgvn, qovn, ktpvn, tsvn, k, f, vybvesch, vybitom);
    k=k+1; tgvnn=vydelPol(thvn, tgvn, qovn, ktpvn, tsvn, k, f, vybvesch, vybitom);
    k=k+1; qovnn=vydelPol(thvn, tgvn, qovn, ktpvn, tsvn, k, f, vybvesch, vybitom);
    k=k+1; ktpvnn=vydelPol(thvn, tgvn, qovn, ktpvn, tsvn, k, f, vybvesch, vybitom);
    k=k+1; tsvnn=vydelPol(thvn, tgvn, qovn, ktpvn, tsvn, k, f, vybvesch, vybitom);
        thvnn=thvnn;
        tsvnn=tsvnn;
...
switch (no)
    case (0)
ktpit(1) = 9e0*ko; ktpit(2) = 12e0*ko;  %из Диссертации
kitom440(2) = (ktpit(2) - ktpit(1)) / (te380 - te200); 
kitom440(1) = ktpit(1) - kitom440(2) * te200;
kti = kitom440;
    case (1)
		ktpit(1) = 12.0*ko; ktpit(2) = 13.9*ko; %из Диссертации
		%ktpit(1) = 18.0*ko; ktpit(2) = 19.0*ko; %Данные 2017 года
		kitom620(2) = (ktpit(2) - ktpit(1)) / (te380 - te200); 
        kitom620(1) = ktpit(2) - kitom620(2) * te380;
		kti = kitom620;
    case (2)
		ktpit(1) = 18.3*ko; ktpit(2) = 19.4*ko; %из Диссертации
		%ktpit(1) = 26.0*ko; ktpit(2) = 37.0*ko; %Данные 2017 года
		kitom860(2) = (ktpit(2) - ktpit(1)) / (te380 - te200); 
        kitom860(1) = ktpit(2) - kitom860(2) * te380;
		kti = kitom860;
    case (3)
		ktpit(1) = 23.0*ko; ktpit(2) = 25.0*ko; %из Диссертации
		%ktpit(1) = 42.0*ko; ktpit(2) = 52.0*ko; %Данные 2017 года
...
%-----
function [ vyma ] = PoiskZavVelTem(v, efte, h, f, vm, vybvesch)
	hf=1e0; e=1e-6;
    n=length(efte);
		...
		for jvsv=1:nnvysv
			...
			if ((vyfrve==0) || (vyfrve==2))
				for qvmi=1:nnvmivmf
                    ...
                    ident=0; temvct=napMasEKTPVermik(vyfrve, vysove, vymivmf, efte, ident, h, vyukve, vm);
                    ident=ident+1; temvht=napMasEKTPVermik(vyfrve, vysove, vymivmf, efte, ident, h, vyukve, vm);
                    ident=ident+1; tepvt=napMasEKTPVermik(vyfrve, vysove, vymivmf, efte, ident, h, vyukve, vm);
                    ident=ident+1; ktp=napMasEKTPVermik(vyfrve, vysove, vymivmf, efte, ident, h, vyukve, vm);
                    ident=ident+1; ts=napMasEKTPVermik(vyfrve, vysove, vymivmf, efte, ident, h, vyukve, vm);
                    fl=-1;
                    k=0; temvctn=vydelPol(temvct, temvht, tepvt, ktp, ts, k, fl, vybvesch, vm);
                    k=k+1; temvhtn=vydelPol(temvct, temvht, tepvt, ktp, ts, k, fl, vybvesch, vm);
                    k=k+1; tepvtn=vydelPol(temvct, temvht, tepvt, ktp, ts, k, fl, vybvesch, vm);
                    k=k+1; ktpn=vydelPol(temvct, temvht, tepvt, ktp, ts, k, fl, vybvesch, vm);
                    k=k+1; tsn=vydelPol(temvct, temvht, tepvt, ktp, ts, k, fl, vybvesch, vm);
                    if (f>0)
                    fl=1;
                    end
            ...
            elseif (vyfrve==1)
				for qvuv=1:nnvyuv
                    ...
                    ident=ident+1; ts=napMasEKTPVermik(vyfrve, vysove, vymivmf, efte, ident, h, vyukve, vm); 
                    j=-1;
                    k=0; temvctn=vydelPol(temvct, temvht, tepvt, ktp, ts, k, j, vybvesch, vm); 
                    k=k+1; temvhtn=vydelPol(temvct, temvht, tepvt, ktp, ts, k, j, vybvesch, vm); 
                    k=k+1; tepvtn=vydelPol(temvct, temvht, tepvt, ktp, ts, k, j, vybvesch, vm); 
                    k=k+1; ktpn=vydelPol(temvct, temvht, tepvt, ktp, ts, k, j, vybvesch, vm); 
                    k=k+1; tsn=vydelPol(temvct, temvht, tepvt, ktp, ts, k, j, vybvesch, vm); 
                    if (f>0)
                    fl=1;
                    end
                    ident=0; temcq=rasMas(efte, temvhtn, temvctn, ktpn, tepvtn, tsn, temvs, temhq, temcq, ktpq, tepv, nomer, vyfrve, vysove, vymivmf, vyukve, fl, ident);
...
%-----
function [ oa ] = opredTempHolGor(ektp, ete, h0, v, efte, vm, vybves) %моделирование процесса теплообмена в образце %n - длина массива ektp, l - длина массивов temvs, qob, ktp, temvh, temvc, ni - длина efte, qob - плотность теплового потока, которую может создать лабораторная установка 
    ektp=ektp;
    ete=ete;
	hf=1e0; te0=273.15; tn=22.0+te0;
    f=1;
	k=2; qon=PoiskZavVelTem(k, efte, h0, f, vm, vybves);
    f=-1;
	k=4; tena=PoiskZavVelTem(k, efte, h0, f, vm, vybves);
    ...
    ktp=zeros(1,ni);
        ...
        temvs=temvs;
    j=-1; 
    k=0; temvcn=vydelPol(temvc, temvh, qob, ktp, temvs, k, j, vybves, vm);
    k=k+1; temvhn=vydelPol(temvc, temvh, qob, ktp, temvs, k, j, vybves, vm);
    k=k+1; qobn=vydelPol(temvc, temvh, qob, ktp, temvs, k, j, vybves, vm);
    k=k+1; ktpn=vydelPol(temvc, temvh, qob, ktp, temvs, k, j, vybves, vm);
    k=k+1; temvsn=vydelPol(temvc, temvh, qob, ktp, temvs, k, j, vybves, vm);
        switch (v)
            case (0)
                vm=temvcn;
            case (1)
...
%-----
function [ oa ] = napMasEKTPVermik(vyfrve, vysove, vymeizvemafr, te, ident, h, vyukve, vm)
	...
	if ((vyfrve==0) || (vyfrve==2)) %фракция 2-0,7 мм или фракция 1,6-0,35 мм
		...
		k=0; koefc=oprkoefKTPiskhchao(k, te, h, vm);
        k=1; koefh=oprkoefKTPiskhchao(k, te, h, vm); 
        k=2; koefq=oprkoefKTPiskhchao(k, te, h, vm);
		k=3; koeft=oprkoefKTPiskhchao(k, te, h, vm);
        k=4; koefs=oprkoefKTPiskhchao(k, te, h, vm);
...
%-----
function [ oa ] = oprkoefKTPiskhchao(v, efte, h, vm, vybvesch) %vmiv - выбор метода измерений %0 - установка Netzsch - нестационарный метод
        mt=arrTem_Netzsch();
		ktpv=arrKTP_Netzsch();
		k=0; thvn=opredTempHolGor(ktpv, mt, h, k, efte, vm);
        k=k+1; tgvn=opredTempHolGor(ktpv, mt, h, k, efte, vm); 
		k=k+1; qovn=opredTempHolGor(ktpv, mt, h, k, efte, vm);
        k=k+1; ktpvn=opredTempHolGor(ktpv, mt, h, k, efte, vm);
        k=k+1; tsvn=opredTempHolGor(ktpv, mt, h, k, efte, vm);
            f=-1;
    k=0; thvnn=vydelPol(thvn, tgvn, qovn, ktpvn, tsvn, k, f, vybvesch, vm);
    k=k+1; tgvnn=vydelPol(thvn, tgvn, qovn, ktpvn, tsvn, k, f, vybvesch, vm);
    k=k+1; qovnn=vydelPol(thvn, tgvn, qovn, ktpvn, tsvn, k, f, vybvesch, vm);
    k=k+1; ktpvnn=vydelPol(thvn, tgvn, qovn, ktpvn, tsvn, k, f, vybvesch, vm);
    k=k+1; tsvnn=vydelPol(thvn, tgvn, qovn, ktpvn, tsvn, k, f, vybvesch, vm);
        thvnn=thvnn;
        tsvnn=tsvnn;
		koefc=koefPribSha(thvnn, tsvnn);
        %le=length(thvnn);
...
%-----
function [ vyma ] = vydelPol(temvcs, temvhs, qos, ektpvs, temvss, v, f, vybves, vybmar)
