function [ PokPrel ] = Kramers_n_Itom620_uk()
format long g;
...
%nnu=PokazPrelomPribl(om,oms,nnus)-1; %constan=1+(1.543-1)*0.5+0.5*(1.56-1); %nnu=nnu+constan; %вермикулит и шамот %n0=postrGraf(nnu,nnus,om,oms);
w1=5e-1; w2=1e0-w1; 
rover=25e1; rosha=219e1;
v1=w1/rover; v2=w2/rosha;
phi1=v1/(v1+v2); phi2=v2/(v1+v2); 
dob=(1.56-1e0)*phi2+(1.543-1e0)*phi1;
nnu=PokazPrelomPribl(om,oms,nnus)+dob; %nnu=vyborka(nnu-constan,Sp);
nnu=vyborka(nnu,Sp);
PokPrel=nnu;
end
