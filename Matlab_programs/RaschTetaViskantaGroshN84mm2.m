format long g; ro=112; cp=1533; y0=3e4*1e-6; y0l=5e3*1e-6; tetan=0; koo=0; tetam=0; koom=0; tau=0; taum=0;
...
tepo=arrTepPot84(); qv=(1/13.85e-4)*tepo; pt=length(temvc); ts=sredtemp(temvh,temvc,pt); dl=dlivoln(); tepv=koeftepv(qv,y0,temvh,temvc,pt);
%-------------
...
r=integrTau(koom(k),np,tetam,y0,Tsr,Nto,alfm(yc),k,ktn);
m=m+r; m=ProvAdek(m); 
if (m==0) 
