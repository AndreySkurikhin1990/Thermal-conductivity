...
switch (vy)
    ...
    case 2 
        lamvy(k)=lamodom(k); %по Одолевскому (матрицы) - 25
    case 3
        lamvy(k)=lamodos(k); %по Одолевскому (стат. смесь) - 26
    case 4 
        lamvy(k)=lamcyladi(k); %цилиндр., адиаб. - 27
    case 5 
        lamvy(k)=lamcylizo(k); %цилиндр., изотерм. - 28
    case 6
        lamvy(k)=lamcylper(k); %ось цилиндра перпендикулярна тепловому потоку - 29
    otherwise
        lamvy(k)=0;
end
