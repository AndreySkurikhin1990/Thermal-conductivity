function t = Kellet_cas_1_Sha(vybsha,vystsha,salosha)
...
for vtem=1:n
ektp=ektpv(vtem)
dko2=opreddko2(temvh(vtem),temvc(vtem))
wmg=0.26e-2; wal=39e-2; wsi=60.74e-2; 
Tz=temvh(vtem); T0=Tz;
Tk=temvc(vtem);
tem=Tk:1:Tz;
koal=rasKoefAlpha(tem,wmg,wsi,wal); n=length(koal);
s=0;
for k=1:n
    s=s+koal(k);
end
...
hisr=knusreddvvsrSha(Tz,hiSha,dv,npp)
...
B=B/(1+exp(-m*d));
C=-B*exp(-m*d);
tetav=PoKo(d, alsr, lamb, sig, teta1, teta2, rosr, teta0, tetad, H)
k=k+1;
end
Nx=1e2;
tetax=0;
...
dko2m=[8.15,6.87,7.55,14.84,20.09,26.3]/1e2; dko2t=6e2:2e2:16e2; 
...
function kor = PoKo(d, alp, lamb, sig, teta1, teta2, ro, teta0, tetad, H)
dt=0;
ta=teta1-dt; %tetad
tb=teta2+dt; %tetad
dete=1e-2;
