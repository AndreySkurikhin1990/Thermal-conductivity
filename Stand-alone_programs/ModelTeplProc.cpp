...
using namespace std;
const int cemn=11, dso=12, dmko=4, isrp=0, vpkf=0, vpmf=0, cemdu=6, cemdum=2; //dmko - äëèíà ìàññèâà êîýôôèöèåíòîâ
...
double **arrTem_Netzsch(double **); //ìàññèâ òåìïåðàòóð - ýêñïåðèìåíòàëüíûå äàííûå íà Netzsch - õàîòè÷íàÿ çàñûïêà ôðàêöèè 2-0,7 ìì (èñõîäíûé)
double *arrKTP_Netzsch(); //ìàññèâ ÊÒÏ âåðìèêóëèòà
double *arrKTP_2020();
double *arrTem1_2020();
...
{	
	...
	if (!vyve) //åñëè âûáðàí øàìîò
	{
	double ko=1e-2, porsha0=21.8*ko, porsha1=11.014*ko, porsha2=25.2*ko, porsha3=26.5*ko, porsha4=11.5*ko, porsha5=16.5*ko; //0 - Ðîó÷êà, 1 - ØÏÄ-41, 2 - ØÁ-1 2-1, 3 - ØÂ-1 1-1, 4 - ØÏÄ, 5 - ØÊÓ-32 3-1
	...
	if ((!vysv) || (vysv==1)) 
	{  //èñõîäíûé èëè ïîñëå ïîâòîðíûõ èçìåðåíèé
	if (!vyfv) 
	{ 
	if (!vpmf) por = por207; 
	else if (vpmf==1) por=por16035; 
	} //âûáîð ïîðèñòîñòè ìåëêîé ôðàêöèè
	else if (vyfv==1) 
	{ 
	if (!vpkf) por = poris84; 
	else if (vpkf==1) por=porin84; 
	} //âûáîð ïîðèñòîñòè êðóïíîé ôðàêöèè
...
double **PoiskZavVelTem(int v, double **mu, int rmu, int n, double *efte, double h)
{
	...
	d=0; temvct=muv[d]; d++; temvht=muv[d]; d++; tepvt=muv[d]; d++; ktp=muv[d]; d++; ts=muv[d]; d++; po=muv[d]; k=0; nf=po[k]; 
					...
					d=1; k=0; cfp=tepvt[k]; for (k=d; k<q; k++) { cft=tepvt[k]; if ((cft<=cfp) && (d>0)) { d=-1; break; } cfp=cft; } //{ for (k=0; k<q; k++) cout << "c = " << c << "\tqo = " << tepvt[k] << "\tts = " << ts[k] << "\t"; cout << endl; } //vyvodfile(ts, q, k, nf, "C:\\Users\\Àíäðåé\\Documents\\_Àñïèðàíòóðà\\tmp\\tmp.txt");
...
int napMasEKTPitomNac()
{
	...
	por=novNapMas(tnac, vyve, vpi, vmivmf, vyfv, vyuv, vysv, cem);
	double hf = 1e0, g=0.0, s=0.0, t=0.0, *po=NULL, nf = 0.0, tn=0.0;
	double *ektpi=new double[cemi], *eteis=new double[cemi], **mu=new double*[f], **muv=new double*[f];
	double *qob=NULL, *thol=NULL, *tgor=NULL, *tsred=NULL;
	if ((!ektpi) || (!eteis) || (!mu) || (!muv)) { cout << snm << endl; k = getchar(); exit(1); }
	...
	muv=opredTempHolGor(ektpi, ete, cemi, k, ys0, k, muv, f, cemi, ete, dmkoi); //cem - äëèíà ìàññèâà efte
	...
	for (k=0; k<cem; k++) cout << "tem = " << ete[k] << "\tktp = " << ektpi[k] << "\tth = " << thol[k] << "\ttg = " << tgor[k] << "\tqo = " << qob[k] << "\t"; cout << endl;
...
int napMasEKTPkviNac()
{
	...
	por=novNapMas(tnac, vyve, vpk, vmivmf, vyfv, vyuv, vysv, cem);
	double hf=1e0, g=0.0, s=0.0, t=0.0, *po=NULL, nf = 0.0, tn=0.0;
	double *ektpk=new double[cemk], *eteis=new double[cemk], **mu=new double*[f], **muv=new double*[f];
	double *qob=NULL, *thol=NULL, *tgor=NULL, *tsred=NULL;
	if ((!ektpk) || (!eteis) || (!mu) || (!muv)) { cout << snm << endl; k = getchar(); exit(1); }
	...
	muv=opredTempHolGor(ektpk, ete, cemk, k, ys0, k, muv, f, cemk, ete, dmkok); //cem - äëèíà ìàññèâà efte
...
int napMasEKTPshaNac()
{
	...
	por=novNapMas(tnac, vyve, vps, vmivmf, vyfv, vyuv, vysv, cem);
	double hf=1e0, g=0.0, s=0.0, t=0.0, *po=NULL, nf = 0.0, tn=0.0;
	double *ektps=new double[cems], *eteis=new double[cems], **mu=new double*[f], **muv=new double*[f];
	double *qob=NULL, *thol=NULL, *tgor=NULL, *tsred=NULL;
	if ((!ektps) || (!eteis) || (!mu) || (!muv)) { cout << snm << endl; k = getchar(); exit(1); }
	for (k=0; k<f; k++) { po=new double[cems]; if (!po) { cout << snm << endl; k=getchar(); exit(1); } muv[k]=po; } 
	for (k=0; k<cems; k++) { t=ete[k]; s=0.0; g=0.0; 
	for (j=0; j<dmkos; j++) { s=s+kektps[j]*pow(t, g); g=g+hf; } ektps[k]=s; } //for (k=0; k<cem; k++) cout << "tem = " << ete[k] << "\tktp = " << ektpi[k] << "\t"; cout << endl;
	muv=opredTempHolGor(ektps, ete, cems, k, ys0, k, muv, f, cems, ete, dmkos); //cem - äëèíà ìàññèâà efte
...
int napMasEKTPVerNac()
{
	int k=1, nk=0, j=0, jk=0, f=cemdu, qn=0, q=0, w=f-1; 
	...
	muv=napMasEKTPVer(vyfv, vysv, vmivmf, ete, k, ys0, vyuv, cem, k, muv, f, dmko); //for (k=0; k<cem; k++) cout << "te = " << ete[k] << "\t"; cout << endl; //k=w; po=muv[k]; k=0; nf=po[k]; t=e; k=0; while (t<nf) { t=t+hf; k++; } cem=k; cout << "cem = " << cem << "\tnf = " << nf << "\n"; for (k=0; k<w; k++) { po=muv[k]; for (j=0; j<cem; j++) cout << " ( " << j << " ) = " << po[j] << "\t"; cout << endl; }
	k=0; j=1; mu=vydelPol(k, cem, muv, mu, f, j);
	k=0; thol=mu[k]; k++; tgor=mu[k]; k++; qob=mu[k]; k++; 
	ektpv=mu[k]; k++; tsred=mu[k]; ete=tsred; k++; po=mu[k]; 
	k=0; nf=po[k]; t=0.0; k=0; while (t<nf) { t=t+hf; k++; } cem=k; //cout << "cem = " << cem << "\tnf = " << nf << "\t";	
	for (k=0; k<cem; k++) cout << "tem = " << ete[k] << "\tktp = " << ektpv[k] << "\tth = " << thol[k] << "\ttg = " << tgor[k] << "\tqo = " << qob[k] << "\t"; cout << endl;
...
double **oprkoefKTPiskhchao(int vmiv, int v, double *efte, double h, int n207, double **mu, int rmu, int cem, int dmk) //vmiv - âûáîð ìåòîäà èçìåðåíèé
{
	...
	if (!vmiv) {  //0 - óñòàíîâêà Netzsch - íåñòàöèîíàðíûé ìåòîä
		...
		muv=opredTempHolGor(ktpv, mt, nk, nn, h, k, muv, rmu, cem, efte, dmko); //cem - äëèíà ìàññèâà efte //for (k=0; k<f; k++) { po=muv[k]; delete[]po; }
...
double *koefPribSha(double *ktp, double *te, int le, double *ko, char *snome)
{
	...
	for (k=0; k<le; k++) 
	{
		...
		x=x+te[k];
		p=p+hf;
	} //ïðèìåíåíèå ìåòîäà íàèìåíüøèõ êâàäðàòîâ
...
double *poisMasKoefsha(double *kektp, int n)
{
	...
	if ((salosha >= 28e-2) && (salosha <= 38e-2)) {
		...
		kektp[3] = -0.867e-9; kektp[2] = 1.77e-6; kektp[1] = -0.523e-3; kektp[0] = 0.806; } //çàäàíèå êîýôôèöèåíòîâ - øàìîò ñðåäíåé ïîðèñòîñòè
		else if ((porsha >= p16) && (porsha < p20)) { vybsha = 1; 
		kektp[3] = -0.397e-9; kektp[2] = 0.71e-6; kektp[1] = 0.011e-3; kektp[0] = 0.851; } //óïëîòíåííûé øàìîò
		else if ((porsha >= p30) && (porsha <= p33)) { vybsha = 2; 
		kektp[3] = -0.377e-9; kektp[2] = 0.918e-6; kektp[1] = -0.338e-3; kektp[0] = 0.77; } //íèçêîïëîòíûé øàìîò
	...
	if ((salosha>38e-2) && (salosha <= 45e-2)) {
		...
		kektp[3] = -0.333e-9; kektp[2] = 0.805e-6; kektp[1] = -0.289e-3; kektp[0] = 0.903; } //çàäàíèå êîýôôèöèåíòîâ - øàìîò ñðåäíåé ïîðèñòîñòè
		else if ((porsha >= p16) && (porsha < p20)) { vybsha = 1; 
		kektp[3] = 0.0; kektp[2] = -0.154e-6; kektp[1] = 0.369e-3; kektp[0] = 1.03; } //óïëîòíåííûé øàìîò
		else if ((porsha >= p30) && (porsha < p33)) { vybsha = 2; 
		kektp[3] = -0.377e-9; kektp[2] = 0.918e-6; kektp[1] = -0.338e-3; kektp[0] = 0.77; }  //íèçêîïëîòíûé øàìîò
		else if ((porsha >= p10) && (porsha < p16)) { vybsha = 3; 
