...
using namespace std;
const int dso=12, dmko=4, isrp=0, vpkf=0, vpmf=0, cemdu=5, cemdum=2; //dmko - äëèíà ìàññèâà êîýôôèöèåíòîâ
...
const double porsha0=21.8*1e-2, porsha1=11.014*1e-2, porsha2=25.2*1e-2, porsha3=26.5*1e-2, porsha4=11.5*1e-2, porsha5=16.5*1e-2; //0 - Ðîó÷êà, 1 - ØÏÄ-41, 2 - ØÁ-1 2-1, 3 - ØÂ-1 1-1, 4 - ØÏÄ, 5 - ØÊÓ-32 3-1
...
double novNapMas(double tnach, int vyve, int vymave, int vmimfv, int vyfrve, int vyukve, int vysove, int cemv)
{
	...
	else if (vyve==1) { //åñëè âûáðàí âåðìèêóëèò
	if ((!vysv) || (vysv==1)) {  //èñõîäíûé èëè ïîñëå ïîâòîðíûõ èçìåðåíèé
	if (!vyfv) { if (!vpmf) por = por207; else if (vpmf==1) por=por16035; } //âûáîð ïîðèñòîñòè ìåëêîé ôðàêöèè
	else if (vyfv==1) { if (!vpkf) por = poris84; else if (vpkf==1) por=porin84; } //âûáîð ïîðèñòîñòè êðóïíîé ôðàêöèè
	else if (vyfv==2) por = poro16035; }
	if (vysv==2) { //ïîñëå îáæèãà
	if ((!vyfv) || (vyfv==2)) por=poro16035;
	else if (vyfv==1) por=poro84; } 
	}
	else if (vyve==2) { //åñëè âûáðàí ÈÒÎÌ
	if (!vymave) por=pori440; else if (vymave==1) por=pori620; 
	else if (vymave==2) por=pori860; else if (vymave==3) por=pori1000;
	}
	else if (vyve==3) { //åñëè âûáðàí ÊÂÈ
...
double **PoiskZavVelTem(int v, double **mu, int rmu, int n)
{
	...
	k=0; nvyuv[k]=1; k++; nvyuv[k]=2; //óêëàäêà âåðìèêóëèòà
...
double **opredTempHolGor(double *ektp, double *ete, int n, int l, double h0, int v, double **mu, int rmu, double *temvs, double *qob, int ni, double *ktp, double *temvh, double *temvc) //ìîäåëèðîâàíèå ïðîöåññà òåïëîîáìåíà â îáðàçöå
{
	...
	if (n<=ni) {
		for (k=0; k<n; k++) {
			...
			qob[k]=g; } //ïëîòíîñòü òåïëîâîãî ïîòîêà, êîòîðóþ ìîæåò ñîçäàòü ëàáîðàòîðíàÿ óñòàíîâêà //for (k=0; k<n; k++) cout << "qob = " << qob[k] << "\tektp = " << ektp[k] << "\tte = " << ete[k] << endl; 
...
double **napMasEKTP(int vyfrve, int vysove, int vymeizvemafr, double *te, int v, double h, int vyukve, int nt, int wa, double **mu, int rmu, int dmk)
{
	...
	if ((!vyfrve) || (vyfrve==2)) { //ôðàêöèÿ 2-0,7 ìì èëè ôðàêöèÿ 1,6-0,35 ìì
		...
		else if (vysove == 3) { //ïîñëå ïîâòîðíîãî îáæèãà ïðè 1000 °Ñ
...
double **oprkoefKTPiskhchao(int vmiv, int v, double *temvs, double *tepv, double h, double *temvh, double *temvc, double *ktp, int n, double **mu, int rmu, int cem) //n=4 - äëèíà ìàññèâà êîýôôèöèåíòîâ, vmiv - âûáîð ìåòîäà èçìåðåíèé
{
	...
	if (!vmiv) {  //0 - óñòàíîâêà Netzsch - íåñòàöèîíàðíûé ìåòîä
	...
	else if (vmiv==1) { //äàííûå 2020 ãîäà - ÃÎÑÒ 12170 - ñòàöèîíàðíûé ìåòîä
		ktp=arrKTP_2020(); 
		tgv=arrTem1_2020(); 
		k=cemdum; muv=new double*[k]; if (!muv) { cout << snm << endl; k=getchar(); exit(1); }
		...
		ts=new double[q]; if (!ts) { cout << snm << endl; k = getchar(); exit(1); } //nk - ðàçìåð ìàññèâà ts
	...
	else if (vmiv==2) { //äàííûå 2019 ãîäà - ÃÎÑÒ 12170
...
double *danIskh207(double *ma, int v, int n, int np)
{
	if (ma) {
		...
		k=0; m[k]=s; k++; p=3; m[k]=ma[p]; k++; p=4; q=5; m[k]=(ma[p]+ma[q])/2e0; //600, 800 è 1000 °C
		delete[]ma; return m;
	}
	else return ma;
}
