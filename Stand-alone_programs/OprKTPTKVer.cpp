...
const int dsnftk=50, hko=10000, dmvtk=28, dmsrps=23;
...
double *DulnevZern(double, double *, double *, int, double *, double *, double, double, double, double, int, double, double, int, double *, double *, double *, double);
...
double *oprKTPTverKarkVermi(double *, double *, double, double, double, double, int, int, double, double, int, double *, double *, double *, int, int, double *, double *, int);
...
double *MetodDulnevaSigalovoyPolyDispVer(double *, int, double, double *, int);
double *MetodDulnevaSigalovoyDopVer(double *, int, double, double *, int);
double *MetodDulnevaSigalovoyBezIspVer(double *, int, double, double *, int);
...
void osvpamtk();
double *DopFunctOpredKTPTverChas(double *, double *, int, double, double *, int, int, double *);
double *NapMasRasLePrVer(int, int, int, int);
double *oprEdinVelitom(int, int, int);
...
//Управляющие функции
double *opredKTPTverKarkVerm(double *tem, double *laefm, double por, double wsio, double walo, double wmgo, int vfv, int n, double tena, double dtos, int dmkusc, double *kuscv, double *tkuscv, double *stchv, int lentem, int vysove, int vpkf)
{ int m=lentem, k=0, dmi=0; 
initpervznach(m, stchv, 0); 
double *srra=NapMasRasLePrVer(vfv, vysove, vpkf, 0);
double *raspr=NapMasRasLePrVer(vfv, vysove, vpkf, 1);
double *dmm=NapMasRasLePrVer(vfv, vysove, vpkf, 2), dm=dmm[0], t=tocras, ht=1e0; while (t<dm) { dmi++; t=t+ht; } //cout << "dmi = " << dmi << endl;
double *ktptsv=oprKTPTverKarkVermi(tem, laefm, por, wsio, walo, wmgo, vfv, m, tena, dtos, dmkusc, kuscv, tkuscv, stchv, vysove, vpkf, srra, raspr, dmi);
osvpamtk(); delete[]srra; delete[]raspr; delete[]dmm; return ktptsv; }
double *oprKTPTverKarkVermi(double *tem, double *laefm, double por, double wsio, double walo, double wmgo, int vfv, int n, double tena, double dtos, int dmkusc, double *kuscv, double *tkuscv, double *stchv, int vysove, int vpkf, double *srra, double *raspr, int dmv)
{ 
int k, cvp=22, cve=cvp+14, cved=cve+4, na=0, j, q, *no=new int[cved], m=dmsrpv, f=(cve-cvp)/2, vv=0, u=n;
double *srk=new double[n], **sr=new double*[cved], *lavo=new double[n], ep=tocras;
double *srzn=new double[n], srp=0.0, s=0.0, p=0.0, srch=0.0, hf=1e0, ht=hf;
if ((!no) || (!sr) || (!lavo) || (!srzn) || (!srk)) { cout << "No memory!"; j=getchar(); exit(1); } for (k=0; k<cved; k++) no[k]=0;
for (k=0; k<n; k++) lavo[k]=opredTeploprovVozd(tem[k]);
srp=srRaPorVerm(m, vfv, vysove, vpkf); if (!vfv) srch=(2e0+7e-1)*1e-3/2e0; else if (vfv==1) srch=(8e0+4e0)*1e-3/2e0;
srk=DulnevZern(por, tem, srk, na, laefm, lavo, wsio, walo, wmgo, srch, n, tena, dtos, dmkusc, kuscv, tkuscv, stchv, porex); sr[na]=srk; na++; 
for (k=na; k<cved; k++) {
srk=new double[n]; if (!srk) { cout << "No memory!"; j=getchar(); exit(1); } for (j=0; j<n; j++) srk[j]=0.0;
    if (k<cvp) srk=opredTvChaKoeTepSrInSpSha(k, por, tem, laefm, n, srk, lavo, srch);
    else if (k<cve) srk=DulnevKoefTepVerm(k, por, tem, n, srk, laefm, lavo, srp, f, srra, raspr, 2, vv, dmv, cvp);
	else if (k<cved) srk=DopFunctOpredKTPTverChas(srk, tem, n, por, laefm, k-cve, vfv, lavo);
    sr[k]=srk; }
for (k=0; k<u; k++) {
	p=0.0; s=0.0; q=0;
	for (j=0; j<cved; j++) if (fabs(sr[j][k])>ep) { s=s+sr[j][k]; p=p+ht; no[q]=j; q++;
	} srzn[k]=s/p; cout << "kttk = " << srzn[k] << "\tlaef = " << laefm[k] << "\t"; } cout << endl;
//j=0; for (k=0; k<cved; k++) { if (k==no[j]) { srk=sr[k]; zapisvfile(srk, u, szfstk); j++; } } 
cout  << "Nomera nenulevyh elementov" << endl; for (k=0; k<q; k++) cout << no[k]+1 << " "; cout << endl << "q = " << q << endl; 
for (k=0; k<cved; k++) { srk=sr[k]; delete []srk; } delete []sr; delete []lavo; delete []no; 
return srzn; }
...
{ double fl; int f=1; 
if (pro<0.0) f=0; else if (pro>(urot*ref)) f=0; else f=1;
...
srla[k]=lam; }
srla=proverkakvi(srla, laefm, lavoz, up, uo, lear); //if ((vyb==4) || (vyb==6) || (vyb==13) || (vyb==14) || (vyb==17)) for (k=0; k<lear; k++) cout << vyb << "\tk = " << k << "\tsr_lam_tt = " << srla[k] << "\n";
return srla; }
...
{ int k, j, m=dmi; double t=0.0, ht=1e0, w=1e0, ep=tocras; //cout << "m = " << m << endl; for (k=0; k<m; k++) cout << "rpr ( " << k << " ) = " << raspr[k] << "\t srra ( " << k << " ) = " << srra[k] << endl; k=getchar();
...
lamvy=proverkakvi(lamvy, lameff, lamvoz, up, uo, n); //if (vy == 7) { for (k=0; k<n; k++) cout << "Lam Kub Adiab 29 = " << lamcubadia[k] << "\tlam = " << lamvy[k] << endl; cout << endl; } //if (vy == 8) for (k=0; k<n; k++) cout << "Lam Kub Izoterm 30 = " << lamcubizo[k] << "\tlam = " << lamvy[k] << endl; //if (vy == 9) for (k=0; k<n; k++) cout << "Lam po Odolevskomu Matr 31 = " << lamvy[k] << endl; if (vy == 10) for (k=0; k<n; k++) cout << "Lam po Odolevskomu Stat Sm 32 = " << lamvy[k] << endl;//if (vy == 11) for (k=0; k<n; k++) cout << "Lam po Cyl Adiab 33 = " << lamvy[k] << endl; if (vy == 12) for (k=0; k<n; k++) cout << "Lam po Cyl Izoterm 34 = " << lamvy[k] << endl; //if (vy == 13) for (k=0; k<n; k++) cout << "Lam po Cyl Perpen 35 = " << lamvy[k] << endl;
...
while ((ra>ep) && (k<nit)) {
	...
	k++; ra=fabs(lama-lamb); } 
return lamc; }
double *DulnevZern(double por, double *te, double *srk, int na, double *laefm, double *lavo, double wsio, double walo, double wmgo, double srch, int n, double tena, double dtosc, int dm, double *kuscv, double *tkuscv, double *stchv, double pormax)
{ int k, f; double uo, up=1e0, epsil, ts, la_voz, la_e, lam, m2, koscve, stchve;
m2=pormax; if (por<porist) uo=opredUrovPodderM03(por); else if (por<5e-1) uo=1.0/(1.0-por); else uo=1.0/(1.0-m2); //cout << "uo = " << uo << "\tpor = " << por << endl;
for (k=0; k<n; k++) {
ts=te[k];
koscve=opredKTPTKTochSha(kuscv, tkuscv, ts, dm);
stchve=opredKTPTKTochSha(stchv, te, ts, n);
epsil=stchve*koscve; 
stch[k]=epsil; 
la_voz=opredKTPTKTochSha(lavo, te, ts, n);
la_e=opredKTPTKTochSha(laefm, te, ts, n);
lam=opredDulnLam1Ver(por, ts, epsil, la_voz, la_e, srch); //cout << "tem = " << tem[k] << "\tlaef = " << la_e << "\tlavo = " << la_voz << endl;
lam=urovOtsechen(lam, la_e, uo);
lam=urovPodder(lam, la_e, up);
srk[k]=lam; } //for (k=0; k<n; k++) cout << "laefm = " << laefm[k] << "\tlavo = " << lavo[k] << "\tlam = " << lam[k] << endl; for (k=0; k<n; k++) cout << "tem = " << te[k] << "\tstch = " << stch[k] << "\tkosc = " << kosc[k] << endl; 
return srk; }
...
double *DopFunctOpredKTPTverChas(double *srk, double *tem, int n, double por, double *laefm, int p, int vfv, double *lavo)
{ if (!p) srk=MetodDulnevaSigalovoyPolyDispVer(tem, n, por, laefm, vfv);
if (p==1) srk=MetodDulnevaSigalovoyDopVer(tem, n, por, laefm, vfv);
if (p==2) srk=MetodDulnevaSigalovoyBezIspVer(tem, n, por, laefm, vfv); 
if (p==3) srk=VasFrayObsh(laefm, lavo, por, n);
double up=1e0, uo=urovPod(por); srk=proverkakvi(srk, laefm, lavo, up, uo, n);
return srk; }
double *MetodDulnevaSigalovoyPolyDispVer(double *tem, int lete, double m22, double *laefm, int vfv)
{ double m20=porex /*общая пористость*/, m2=m20-m22 /*m2 - межзерновая пористость (внешняя), m22 - внутрення пористость*/;
double r, A=pow(74e-2/(1e0-m2),1e0/3e0); 
...
if (m20<porist) uo=opredUrovPodderM03(m20); else uo=1e0/(1e0-m20);
if (!vfv) r=(2e0+7e-1)*1e-3/2e0/2e0; 
else if (vfv==1) r=(8e0+4e0)*1e-3/2e0/2e0; 
else if (vfv==2) r=(1.6+0.35)*1e-3/2e0/2e0;
for (k=0; k<lete; k++) {
...
izco=2e0*(5.67e-2)*pow(stch[k],2e0)*3e0/4e0;
	...
	if ((fc*fa>0.0) && (fb*fc<0.0)) lada=ladc;
	ra=fabs(fa-fb); h++; }
ladc=urovOtsechen(ladc, laef, uo); ladc=urovPodder(ladc, laef, up);
ladi[k]=ladc; } return ladi; }
double *MetodDulnevaSigalovoyDopVer(double *tem, int lete, double m22, double *laefm, int vfv) /*m22 - пористость вермикулита фракции 2-0,7 мм, из статьи "Теплопроводность моно- и полидисперсных материалов"*/
{ double m20=porex /*общая пористость*/, m2=m20-m22, r;
...
if (m20<porist) uo=opredUrovPodderM03(m20); else uo=1.0/(1.0-m20);
if (!vfv) r=(2e0+7e-1)*1e-3/2e0/2e0; 
else if (vfv==1) r=(8e0+4e0)*1e-3/2e0/2e0;
else if (vfv==2) r=(1.6+0.35)*1e-3/2e0/2e0;
for (k=0; k<lete; k++) {
...
ladc=urovOtsechen(ladc, laef, uo); ladc=urovPodder(ladc, laef, up);
ladi[k]=ladc; } return ladi; }
double *MetodDulnevaSigalovoyBezIspVer(double *tem, int lete, double m22, double *laefm, int vfv) //m22 - пористость вермикулита
{ double m20 = porex /*общая пористость*/, m2=m20-m22, r; //из статьи "Теплопроводность зернистых систем"
...
ar[k]=1.06; k++; ar[k]=1.029; k++; ar[k]=1e0; k++; ar[k]=0.972; k++; ar[k]=0.946; k++; ar[k]=0.898; k++;
...
if ((fc*fa>0.0) && (fb*fc<0.0)) lama=lamc; 
h++; ra=fabs(lama-lamb); } 
lamc=urovOtsechen(lamc, laef, uo);
lamc=urovPodder(lamc, laef, up);
lto[k]=lamc; }
return lto; }
...
{ int k, f; double lvmi=1e0; for (k=0; k<n; k++) if (lvmi>lavo[k]) lvmi=lavo[k];
for (k=0; k<n; k++) {
