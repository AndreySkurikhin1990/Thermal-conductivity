...
#include <windows.h>
#include <iostream>
#define vmivmf 1 //âûáîð ìåòîäà èçìåðåíèé äëÿ ôðàêöèè 2-0,7 ìì: 0 - íåñòàöèîíàðíûé, 1 - ñòàöèîíàðíûé
#define vyfv 1 //âûáîð ôðàêöèè: 0 - ôðàêöèÿ 2-0,7 ìì, 1 - ôðàêöèÿ 8-4 ìì, 2 - ôðàêöèÿ 1,6-0,35 ìì
...
const int vtvn = 0, isrp=0, vpkf=0, vpmf=0; //0 - ñòàðûå, 1 - íîâûå
...
//-----
int main()
{ int k=0; //int j, jk = nxtv; double smgv=smgv84, ssiv=ssiv84, salv=salv84; initarrver(jk, smgv, ssiv - pksvv, salv + pksvv); //k=napMasEKTPVerNac();
k=napMasEKTPshaNac(); //k=napMasEKTPkviNac(); //k=napMasEKTPitomNac();
k=getchar();//int dmi=0; double **mu=rasPorpoRazVer(porver, vyfv, k, vysv, isrp, vpkf), *po; //0 - ñòàðûå, 1 - íîâûå çíà÷åíèÿ //for (k=0; k<6; k++) { po=mu[k]; if (po) delete[]po; } if (mu) delete[]mu; 
return 0;
}
void novNapMas()
{
...
double vychopred(double  **mas, int m) {
  double d=Determinant(mas, m); //Âû÷èñëåíèå îïðåäåëèòåëÿ
  return d; }
double Determinant(double **mas, int m) { // Ðåêóðñèâíîå âû÷èñëåíèå îïðåäåëèòåëÿ
  int i;
  ...
  if (m>2) {
	for (i=0; i<m; i++) {
		...
		d=d+k*mas[i][0]*Determinant(p, m-1); //ðàçëîæåíèå ïî ñòðîêàì ïî ïåðâîìó ñòîëáöó
		k=-k; } } //(-1) â ñòåïåíè i
for (i=0; i<m; i++) { po=p[i]; delete []po; } return (d); }
double **GetMatr(double **mas, double **p, int i, int j, int m) { //Ïîëó÷åíèå ìàòðèöû áåç i-é ñòðîêè è j-ãî ñòîëáöà
  int ki, kj, di, dj;
  di = 0;
  for (ki = 0; ki<(m-1); ki++) { // ïðîâåðêà èíäåêñà ñòðîêè
    if (ki == i) di = 1;
    dj = 0;
    for (kj = 0; kj<(m-1); kj++) { // ïðîâåðêà èíäåêñà ñòîëáöà
      if (kj == j) dj = 1;
